...
python -m conformance.runner                           # Results: 346 passed, 0 failed, 346 total
...
- `ball-lang-engine` — self-hosted engine (SKILL.md Phase 4 Option B), same approach as TS/C++:
  ...
  with Dart-identical output (`Results: 346 passed, 0 failed, 346 total`; the 4 golden-less
...
- `Ball.Engine` (`engine/`) — self-hosted engine (SKILL.md Phase 4 Option B), same approach as
  ...
  Dart-identical output (`Results: 346 passed, 0 failed, 346 total`; the 4 golden-less
...
- `go/engine` — self-hosted engine (SKILL.md Phase 4 Option B), same approach as TS/C++/Rust/C#:
  ...
  conformance corpus with Dart-identical output (`Results: 346 passed, 0 failed, 346 total`; the 4
...
- `python/engine` — self-hosted engine (SKILL.md Phase 4 Option B), same approach as
  ...
  compiled engine runs the whole conformance corpus with Dart-identical output (`Results: 346
  passed, 0 failed, 346 total`; the 4 golden-less resource-limit/sandbox fixtures are documented
