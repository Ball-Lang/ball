//! `ball-lang-engine` — runs Ball programs via the self-hosted engine (issue #39).
//!
//! ## Design: self-host, mirroring the TS/C++ targets
//!
//! Rather than hand-writing a tree-walking interpreter, `ball-lang-engine` follows
...
//! (`rust/engine/tool/`) compiles through `ball-lang-compiler` into
...
//! ball-lang-engine --features self_host`; the `self_host_run` integration tests).
...
//! `catch_unwind` closure via `ball_lang_shared::runtime::BallFlow`), the
...
// not yet compile through `ball-lang-compiler` (see "Self-host status" above), so
...
pub enum EngineError {
    ...
    /// `ball-lang-compiler` (issue #39; see the crate doc comment's "Self-host
...
impl BallEngine {
    ...
    pub fn run(&self) -> Result<Vec<String>, EngineError> {
        ...
        #[cfg(not(feature = "self_host"))]
        {
            Err(EngineError::SelfHostPending(
                "compiled_engine.rs compiles through ball-lang-compiler but does not \
    ...
    fn run_self_hosted(&self) -> Result<Vec<String>, EngineError> {
        ...
        use ball_lang_shared::{BallFunction, BallMap, BallMessage};
        ...
        match outcome {
            ...
            Err(payload) => {
                let value = ball_lang_shared::runtime::ball_catch_payload(payload);
