...
## What capability analysis proves (and the blind spots left)
...
**Same-named decoy shadow — surfaced (#420, closes the #402 residual).** The bare-name capability fallback still deliberately skips a call whose name **is** a declared non-base user function (so a genuine user call is never a false positive). A program can therefore declare a same-named **decoy** user `mutex_create` and the call dispatches to that user function, not the base — so it carries no `concurrency` capability and `--deny concurrency` does not (and should not) fire. What changed in #420 is that such a declaration is **no longer reported as clean**: `analyzeCapabilities` now emits a `shadows` list and `auditReport` renders a **"Shadowed base functions"** section naming each colliding function and the capability the shadowed base fn would carry, escalating an otherwise-pure program's summary from "NO RISK — pure computation only" to **"REVIEW REQUIRED — declares base-function shadows"**. Only shadows of *capability-bearing* (non-`pure`) base names are reported; reuse of a pure base name (e.g. a user `add` alongside `std.add`) stays silent. **Require `@ball-lang/cli`/`@ball-lang/engine` ≥ the #420 release** to get the shadow section. Engine defense-in-depth also landed: the Dart engine now **fails loud** (`Ambiguous unqualified call … shadows a base function …`) if an *unqualified* call resolves by bare name to a name declared as BOTH a base and a user function — the order-dependent ambiguity a decoy could exploit if dispatch order ever changed. The narrower #402 case (a spoofed *non-empty* `call.module` naming an undeclared base module, with **no** decoy) still executes on the engine and remains separate deferred hardening. The robust answer is still not audit alone: pair it with a **deny-by-default engine module allowlist** (Dart `moduleHandlers` — never register `std_io`/`std_fs`/`process`/`memory`/`concurrency`/net for untrusted programs). On TS, where no run-time allowlist exists, keep the input surface minimal and the audit mandatory.

**Custom `isBase` modules are surfaced, not ranked (#609).** A program may declare its own base module — structurally identical to how `std_fs.file_read` is declared, just with a name the table does not know — and have the host implement it through a `BallModuleHandler`. Until #609 that was invisible: the `"module.function"` lookup missed, the bare-name fallback missed (the name is in no std module), the call was filed as an ordinary user call, and a program calling `evil.exfiltrate` reported **"NO RISK — pure computation only"** with `--deny` passing it. Every call into a DECLARED base function the capability table does not model now yields an explicit `custom` capability: risk `unknown`, the call site listed as `main.main → evil.exfiltrate`, the summary escalated to `REVIEW REQUIRED — calls into custom base modules`, `--deny custom` enforceable, and an `Unknown Termination` info in the termination half saying the callee cannot be analyzed. The classification keys on the DECLARATION, not on the spoofable call-site string, so it never invents a capability a program did not declare. It also resolves the CALL by function identity the way the engine does, not by the call-site module: an unqualified call (`{module: "", function: "exfiltrate"}`) or one under a benign-looking module name reaches the same host handler through the engine's bare-name scan, so it is classified the same way. When the two differ the report and the `--deny` violation lead with the declaring module and append the call-site spelling (`evil.exfiltrate (call site: main.exfiltrate)`), and `--output` JSON carries the declaring module as `CallSite.resolvedModule` so an automated consumer is not misled either. Both directions still fail closed: an undeclared name stays an ordinary user call, and a bare name a non-base user function also declares is not resolved (the engine refuses to dispatch that case at all — the #420 ambiguity guard throws). **What it does NOT give you:** any statement about what that host function does. `custom` means "this program calls out to your code", not "this is safe" and not "this is dangerous" — the allowlist walk below is still mandatory, and a vetted module of your own (a `ui`) will appear under `custom` too, by design. **Squatting a std name is closed too (#683).** #609 keyed the classification on the module NAME — anything outside the eight `std*` names — which left a hole: a program-supplied module literally called `std` declaring `exec_shell` was read as std territory, fell through to an unresolved user call, and audited **pure / NO RISK**. The test is now what `capability_table.dart` MODELS, so any declared base function the table does not key is `custom`, whatever module declares it. The #402 bare-name resolution stays scoped to the eight std module names on purpose: the conformance corpus labels real base functions loosely (`std.list_push` for `std_collections.list_push`), while a host module declaring `mutex_create` must not be blessed by that coincidence. The table itself is now a CI-gated CLOSED SET (`dart/shared/test/capability_table_closed_set_test.dart`): every base function the eight `buildStd*Module()` builders declare must be keyed, and every base function an executed conformance fixture declares must resolve — before #683 sixty-odd declared, dispatched and executed functions (`std.type_of`, `std.to_int`, `std_collections.list_join`, …) were simply absent, which is what made the hole unfixable. **Minimum version:** #683 landed after #609 (2026-09-13); an older build reports a squatter as pure.

**`Summary:` precedence — pinned, and NOT an enforcement surface (#682).** Exactly one summary line is printed, first match wins: `NO RISK` → `REVIEW REQUIRED — declares base-function shadows` → `REVIEW REQUIRED — calls into custom base modules` → `HIGH RISK` → `MEDIUM RISK` → `LOW RISK`. The `custom` arm sits **above** `HIGH RISK` deliberately: a program calling both `std_io.exit` and a host module prints `REVIEW REQUIRED`, not `HIGH RISK`, because an effect the report cannot bound outranks one it can, and printing `HIGH RISK` would assert a ceiling nothing proved. **So never grep the summary line for `HIGH RISK`** — that substring is absent from a program that is high-risk AND custom. Enforce with `--deny <capability>` or by reading `capabilities[]`; both still fire on every call site of both capabilities, and both are unaffected by this ordering. (`.github/actions/ball-audit/action.yml` greps only for `NO RISK`, which this arm correctly withholds.)

**The remaining blind spot: declarative `module_imports`.** `analyzeCapabilities` walks call expressions in function bodies, **not** the `module_imports[]` on a `Module` (`ball.proto:148-162`, sources `http`/`git`/`registry`/`inline`). A program can pull in additional code declaratively and the capability report will not mention it. The engine's optional `ModuleResolver? resolver` is what would fetch `http`/`git`/`registry` sources on demand — leave it null for untrusted input, and reject non-empty imports outright.

**Mitigation (the import blind spot, and your own module allowlist), as runnable code — not a comment:**
...
| `custom` | unknown | every call into a base function the PROGRAM declares that `capability_table.dart` does not model (#609, #683) |
