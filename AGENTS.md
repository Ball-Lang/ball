...
- **Go** (epic #426) — a complete pipeline: proto bindings (`go/shared`, package `ballv1`) + a zero-dependency runtime value model (`go/runtime`, package `ballrt`), a Ball → Go compiler (`go/compiler/`), a `go/parser`-based Go → Ball encoder (`go/encoder/`, universal `std`, no `go_std`), a self-hosted engine that runs the whole conformance corpus at Dart parity (`go/engine/`, `Results: 347 passed, 0 failed, 347 total`, behind the off-by-default `selfhost` build tag), and a `ball` CLI (`go/cli/`, #437: `run`/`compile`/`encode`/`check`, plus the self-hosted cli-core verbs `info`/`validate`/`tree`/`version` behind the off-by-default `clicore` build tag, #570). CI is the `go` job in `ci.yml` (build/vet/gofmt/test + the regenerate-and-run self-hosted engine sweep + the cli-core golden-parity gate) plus a `go-engine` row in `conformance-matrix.yml` (#426 Phase 7) — see `go/AGENTS.md`.
