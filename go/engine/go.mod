...
// compiler into compiled/compiled_engine.go, then driving it with a thin native
// wrapper. The Go sibling of rust/engine, csharp/engine, ts/engine.
//
...
// (conformance/roundtrip.go). The compiled engine artifact lives under
// compiled/ and is COMMITTED with no build tag since #586 — `go install` serves
// the repository at the tag and accepts no `-tags`, so a gitignored artifact
// would never reach a consumer (see engine/AGENTS.md).
