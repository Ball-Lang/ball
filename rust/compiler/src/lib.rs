...
impl<'a> Compiler<'a> {
    ...
    fn compile_lambda(&self, lambda: &FunctionDefinition) -> String {
        ...
        self.pop_scope();
        // A lambda is a first-class function *value*: wrap the compiled
        // `move` closure in a `BallValue::Function` so it can be stored in a
        // scope/map, returned, passed as a callback, or compared `is
        // Function` — and later invoked uniformly via `ball_call_function`
        // (issue #39, gap #6). The closure is `Send + Sync + 'static` (it
        // captures owned, already-`.clone()`d `BallValue`s), which
        // `BallFunction`'s `Arc<dyn Fn … + Send + Sync>` requires — keeping
        // `BallValue` `Send` so `ball_throw`'s `panic_any` still type-checks.
