...
| `ball_protobuf_rt.h` | **Generated — regenerate, don't hand-edit.** Ball's own `ball_protobuf` runtime, compiled Ball→C++ in `--library` mode (header-only, self-contained: only `<std>` headers, NO libprotobuf/abseil/nlohmann). Regenerate via `dart/compiler/tool/compile_ball_protobuf_cpp.dart`, or directly: `ball_cpp_compile dart/shared/ball_protobuf.json --library --ns ball_protobuf --out cpp/shared/ball_protobuf_rt.h`. **CI-gated since #708** — see "Freshness" below. |
...
## Freshness (`ball_protobuf_rt.h` is regenerated and diffed — #708)
`ball_protobuf_rt.h` is the C++ target's only COMMITTED generated artifact, and
until #708 it was the only committed generated artifact in the repo that no job
regenerated and diffed. It drifted for four months: last regenerated at #398, it
still carried the two-argument `ball_cast_assert` that #659 replaced everywhere
else and none of the `sink` handling #630 added — because `ball_cpp_compile`
splices its runtime preamble into every file it emits, and this file's copy of
that preamble froze at #398.

The gate is two steps in `ci.yml`'s `cpp` job (Linux leg), right after
`Build all targets`:
...
git diff --exit-code -- cpp/shared/ball_protobuf_rt.h
```

There is exactly ONE such gate for this artifact — do not add a second
regeneration pass elsewhere. It lives in the `cpp` job rather than
`Ball Artifact Freshness` because the generator IS `ball_cpp_compile`, which
only this job builds; `ball-freshness` installs Dart, Node and Go and no C++
toolchain.

When the gate goes red it uploads the regenerated bytes as the run's
`regenerated-cpp-protobuf-rt` artifact (the `cpp`-job sibling of #619's
`regenerated-artifacts`), so a lane without a C++ toolchain can apply the fix:

```bash
gh run download <run-id> --repo Ball-Lang/ball \
  --name regenerated-cpp-protobuf-rt --dir /tmp/rt
...
protobuf engine never started this job at all; `detect-changed-stacks`'
`ball_protobuf_src` signal (#708) now ORs them into `cpp`, with truth-table rows
both ways.

Its raise sites are in scope for `tools/check_error_rendering_tables.py` too:
that checker's C++ glob is `cpp/shared/include/*.h` **and** `cpp/shared/*.h`
since #708, so a Dart error name added to `dart/ball_protobuf/lib/**` and
cross-compiled into this header must be one the cross-target contract knows.
