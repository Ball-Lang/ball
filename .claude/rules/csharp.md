...
(`Results: 362 passed, 0 failed, 362 total (4 skipped carve-outs)`; the 4 golden-less
...
- **A caught `TypeError` reads as Dart's own message, and the rendering table is
  ...
  source on the SDK. `BallRuntime.PatternCastAssert` takes the subject (`BallValue`) now,
  and `BallValue.DartErrorToString` gained the `TypeError` arm with an EMPTY
  prefix.
...
- **A USER-thrown built-in error reads the same on every target, and the table
  ...
  This target needed only the `ArgumentError` row: `BallThrow`'s untyped
  constructor (#615) already aliases `arg0` to `message`.
  `csharp/compiler/test/UserThrownBuiltinErrorTests.cs` drives a real
  `BallRuntime.Throw` so the alias is pinned rather than assumed.
...
- **`ArgGet`'s two operands are a TOLERANT `std_collections.map_get`, never `field_access`
  ...
  with `RuntimeHelpers.CollectionsTable`, the 48-helper `std_collections` family. The measured
  first blockers now head with `BallRuntime.Iterate` (the `foreach` lowering), `CallMethod` and
  `MapCreate`, plus the un-inverted `new BallFunction` a Ball lambda compiles to — read the live
  number and the live first blocker off that row, never off this line.
...
- **Status: complete, runs at Dart parity** (#383/#384 closed). `Results: 362 passed, 0 failed,
  362 total (4 skipped carve-outs)` — the whole conformance corpus, matching Dart's output
