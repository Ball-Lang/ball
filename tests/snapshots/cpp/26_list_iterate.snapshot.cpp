...
int main() {
    auto xs = BallDyn(std::vector<int64_t>{1LL, 2LL, 3LL, 4LL, 5LL});
    auto product = BallDyn(1LL);
    for (auto i = 0LL; (i < ball_length(xs)); ball_assign(i, (i + 1LL))) {
        ball_assign(product, (product * static_cast<BallDyn>(xs)[i]));
    }
    std::cout << ball_to_string(ball_to_string(product)) << std::endl;
    auto total = BallDyn(0LL);
