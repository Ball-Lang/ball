...
- `@ball-lang/cli` (`cli/`) — CLI entry point. `run` executes via `@ball-lang/engine`; `info`/`validate`/`tree`/`version` are self-hosted (`compiled_cli.ts`, compiled from `dart/shared/lib/cli_core.dart` the same way `compiled_engine.ts` is — see "Regenerate compiled TS CLI core" above), wrapped by a small `cli_core.ts` normalizer; `audit` is still a hand-ported `capability_analyzer.ts` (issue #362 residual — `cli_core.dart`'s own `auditReport` isn't self-hostable yet). `index.ts` `await import()`s each compiled engine lazily, inside its own command handler — never at module top level — because two independently-compiled Ball TS artifacts loaded in the same process corrupt each other's `Map.prototype` monkey-patch (see `ts/cli/AGENTS.md`).
