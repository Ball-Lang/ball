...
- **Go** (epic #426) — a compiler + encoder + self-hosted engine pipeline: proto bindings (`go/shared`, package `ballv1`) + a zero-dependency runtime value model (`go/runtime`, package `ballrt`), a Ball → Go compiler (`go/compiler/`), a `go/parser`-based Go → Ball encoder (`go/encoder/`, universal `std`, no `go_std`), and a self-hosted engine that runs the whole conformance corpus at Dart parity (`go/engine/`, `Results: 320 passed, 0 failed, 320 total`, behind the off-by-default `selfhost` build tag). CI is the `go` job in `ci.yml` (build/vet/gofmt/test + the regenerate-and-run self-hosted engine sweep) plus a `go-engine` row in `conformance-matrix.yml` (#426 Phase 7) — see `go/AGENTS.md`. The unified `ball` Go CLI is a pending later phase.
