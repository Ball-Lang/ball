...
/// Read `path` as Rust source, encode it via `ball-encoder`, and write the
/// resulting `ball.v1.Program` to `output` (or stdout) in `format`.
///
/// `ball_encoder::encode` `panic!`s/`assert!`s on source it doesn't support
...
pub fn encode(path: &Path, output: Option<&Path>, format: Format) -> Result<(), CliError> {
    ...
    let program = catch_panic_message(|| ball_encoder::encode(&source))?;
    match format {
