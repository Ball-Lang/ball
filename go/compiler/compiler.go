...
// (no func main) — used for the self-hosted engine (epic #426 Phase 4) and the
// CLI core (issue #570).
//
// The emitted library carries NO build constraint. It used to stamp
// `//go:build selfhost`, because both generated artifacts were gitignored and
// therefore had to stay invisible to a default build of a fresh checkout. Since
// #586 they are TRACKED — a `go install`-acquired `ball` must be able to run
// programs and the cli-core verbs, and `go install` accepts no `-tags` — so a
// constraint would only hide them from the builds that need them.
