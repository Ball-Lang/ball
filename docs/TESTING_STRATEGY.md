...
6. **A harness that can hang forever is itself a defect, and its budget is
   ...
   Python's is `BALL_TIMEOUT_S`) and fails loud on a non-integer value, and the
   two halves are pinned SEPARATELY, both on every PR: the pure
   `the_per_fixture_budget_is_read_from_ball_timeout_ms` owns the env spelling,
   the unset default and the loud typo rejection, while
   ...
   production path with the budget handed to `run_dart` as an ARGUMENT. Splitting
   them is not cosmetic: libtest runs a target's non-`#[ignore]`d tests
   concurrently, so a self-test that SET `BALL_TIMEOUT_MS` would be writing the
   environment out from under its neighbours, and `std::env::set_var` is `unsafe`
   for exactly that reason. That target carries `#![forbid(unsafe_code)]` so the
   ban is a build error rather than a comment a later reader has to honour —
   the SAFETY note that justified the old `set_var` (“the only
   non-`#[ignore]`d test in this target”) was false when it was written, and
   a rule that only prose enforces rots exactly that way (#790).
...
| **The per-fixture kill actually kills** (#693) | `rust/engine/tests/roundtrip_conformance.rs`'s `a_runaway_fixture_is_killed_at_the_budget_and_reported_as_a_timeout` — builds a fabricated runaway with `rustc` at test time (ignores its arguments, never exits), drives it through the real `run_dart` path, and asserts the `__timeout__` sentinel comes back inside the budget it is HANDED — `run_dart` takes the budget as an ARGUMENT, so the kill is proven in milliseconds with no environment write. That seam is the point: libtest runs this target's five non-`#[ignore]`d tests concurrently (the whole-corpus sweep beside them is `#[ignore]`d and runs only under `--ignored`), so a `set_var` here would be writing the environment out from under its neighbours. The env SPELLING is pinned separately, by the pure `the_per_fixture_budget_is_read_from_ball_timeout_ms` | every PR (the `rust` job's `cargo test --workspace`) |
