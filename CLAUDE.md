...
- **Self-hosted engine** (`dart/self_host/lib/engine_rt.cpp`): The Dart engine compiled to C++ via the Ball C++ compiler. Compiles and runs — 60/135 conformance tests pass (44%). Key fixes applied: (1) scope mutations now use pass-by-reference semantics so for/while loop variables update correctly; (2) shared_ptr scopes for closure capture; (3) try/catch properly catches BallException with type matching; (4) `ball_set` unwraps BallDyn from std::any (MSVC wraps instead of using operator std::any()). Remaining issues: (a) MSVC BallDyn-in-std::any wrapping still affects OOP/class tests (~30 tests); (b) 3 tests skipped for infinite loops (memoization with BallDyn map keys); (c) some std functions missing or not fully working. The conformance test suite runs ALL 135 programs with per-test 5s timeout. Build: `cmake --build build --target test_selfhost_conformance`.
