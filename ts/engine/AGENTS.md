...
- **Being committed makes it the ONE compiled engine that can go stale in git** — every other target rebuilds its own from source inside its CI job. Two gates now cover that: ci.yml's `Ball Artifact Freshness` job regenerates it and diffs (`Assert compiled TS engine is up to date`), and `test/compiled_engine_parity.test.ts` locks the behaviour. The corpus alone cannot: it is generated from Dart by the Dart encoder, so it only exercises the Ball shapes that encoder emits — a stale artifact carrying an older `_isBareSelfConstruction` rule (#499) left all 371 fixtures green while this engine infinitely recursed on a program the Dart engine ran fine.
