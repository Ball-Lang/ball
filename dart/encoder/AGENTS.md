...
- **A built-in accessor route yields to a member the receiver's own type declares** (#697 A, `_userMemberShadowsBuiltinAccessor`). `DartEncoder.builtinAccessorGetters` is the CLOSED SET of the ten getter names the encoder diverts away from a plain `fieldAccess` — `_directGetterRoutes`' seven (`sign`/`isNaN`/`isFinite`/`isInfinite`/`isEmpty`/`isNotEmpty`/`runes`, one `std` call each) plus the three composites (`isEven`, `isOdd`, `reversed`). It is the single source of truth for that routing, and `test/builtin_accessor_user_member_test.dart` derives one case per entry, so a route added without the seam fails that gate with no test edit. That suite bounds the set two ways on purpose (#786): the per-name matrix reads the REAL table, while `closedSetComplaints` compares it for exact SET EQUALITY against an independently restated list of the ten names — a count, even one pinned at ten, does not move for a swap (`isEven` out, `isBlank` in) or a rename, so it would let the matrix iterate a different population in silence. The gate is a function precisely so the suite can feed it a deletion, a swap, a rename and an addition and prove it rejects each; adding a route costs one line in `expectedBuiltinAccessorGetters` plus the "ten getter names" prose here, in `.claude/rules/dart.md` and in `docs/TESTING_STRATEGY.md`. The seam suppresses a route only on PROOF, by either of two routes, because the encoder runs in two modes: RESOLVED (`InterfaceType.lookUpGetter` resolves the member to a declaration outside the SDK — the `prepareStaticTypes()` half) or SYNTACTIC (the receiver's declared type name is a class/mixin/enum/extension-type THIS unit declares that declares the member, walking `extends`/`with`/`implements`/`on` inside the unit; the type name comes from an instance creation, an unresolved-AST constructor call, `this`, or the nearest binding of a simple identifier — local, formal parameter, field, top-level variable, mirroring `_isSinkReceiver`'s walk). Neither proof ⇒ the route stands exactly as before. `this` and `super` are the two receivers whose type needs no search — the enclosing declaration, and its `extends` clause — and the first cut of the seam consulted neither, which is why `this.isEmpty` inside the class that declares `isEmpty` still routed. `_enclosingThisTypeName` answers `this` and sits inside `_syntacticReceiverTypeName`, so the binding walk reaches `var me = this; me.isEmpty` too; inside an EXTENSION it answers the `on` type, never the extension, because `extension R on String { … this.isEmpty … }` IS `String.isEmpty` and must stay routed. `super` needs its own channel (`_enclosingSuperTypeNames`, consulted directly by the seam): it denotes a different type from the enclosing declaration, a mixin's `on` clause may name several, and the enclosing declaration's OWN members must never stand in for its superclass's. An extension type's REPRESENTATION parameter is a deliberate, pinned decline — the encoder models it nowhere (neither descriptor nor `metadata['fields']`), so only its body members are consulted. The SYNTACTIC half is what makes this reach `encode(String)` — the mode `generate_conformance.dart` and every self-host regeneration use — so like the null-aware chain fix below it CAN move the committed artifacts (it did not: no class in `engine.dart`/`cli_core.dart` declares one of the ten names — regenerate rather than assume).
...
`<module>` is the DECLARING module, resolved from the override's own element
(`_extensionOwnerModule`), so an extension living in another file of the package
works and so does an import PREFIX — a prefix is a spelling of the same library,
not a different selection. `PackageEncoder.prepareStaticTypes()` supplies the
`library URI → module` map that makes this possible; a per-file `DartEncoder`
without it can still name only the extensions THIS module declares.

REFUSED, loudly, for every override this encoder cannot name soundly:

* an extension **no module of this encode declares** (an external package's, or
  one in a directory `PackageEncoder` does not scan) — there is no Ball function
  to name;
* explicit type arguments ON THE EXTENSION (`Ext<int>(x)` — the call's
  structured `type_args` renders on the MEMBER, a different instantiation);
* a NULL-AWARE override (`Ext(x)?.m()`). The override branch runs BEFORE the
  encoder's null-aware lowering, so encoding it here drops the `?` and calls the
  member on `null` — "skip it" silently becoming "do it". Lowering it needs the
  receiver let-bound so the guard does not evaluate it twice; until then it is
  refused like every other shape that cannot be carried faithfully.

A refusal warns naming the construct and leaves the `/* unsupported: … */`
placeholder, which breaks the front end. Never erase an override to
`receiver.member`: #670 MEASURED that repair as 4 real test failures on
`collection` — a loud build error traded for a silently wrong answer.
...
The ENGINES dispatch `<module>:<Ext>.<member>` by NAME, like any other module
function — nothing target-specific mangles it — so the same suite RUNS the
encoded cross-module program on the reference engine and compares stdout with
what Dart itself prints. That is the cross-target statement available without a
conformance fixture; a fixture is still owed by #670, and is blocked on the
non-Dart COMPILERS, none of which reads `kind: 'extension'` at all yet.
