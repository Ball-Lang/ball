...
## Purpose
Ball → C++ code generator. Walks a protobuf-free `ball::ir::Program` tree (loaded via nlohmann/json — #18 Stage 4/5) and emits self-contained C++ source as one translation unit (#601 removed the split multi-TU sibling).
...
| `include/compiler.h` | `CppCompiler` class declaration; `CompileLibraryResult` struct |
| `include/code_builder.h` | Line-oriented string builder used throughout emission |

## For AI Agents
- Entry point: `CppCompiler::compile()` (the whole program, one TU), `compile_module()` (one module) or `compile_library()` (a Module facade → header-only library). Read `compiler.h` first.
