...
namespace ball {
inline constexpr const char* BALL_DYN_SOURCE =
R"BALL_RT(// ball_dyn.h -- Dynamic value type for Ball compiled programs.
//
...
public:
    ...
    BallDyn operator[](const std::string& key) const {
        ...
        if (_val.type() == typeid(Bal)BALL_RT"
R"BALL_RT(lUMap)) {
    ...
    // Comparison operators for order)BALL_RT"
R"BALL_RT(ing
    bool operator<(const BallDyn& o) const {
        if (_val.type() == typeid(int64_t) && o._val.type() == typeid(int64_t))
...
inline std::regex ball_to_regex(const BallDyn& d) { return)BALL_RT"
R"BALL_RT( std::regex(ball_to_string(d)); }
...
inline int64_t ball_length(const std::map<K, V>& m) {)BALL_RT"
R"BALL_RT( return static_cast<int64_t>(m.size()); }
