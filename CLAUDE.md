...
`v0.1.0` and the local pins live in `go/go.work`'s versioned `replace` block.
`tools/go-module-proxy/smoke.sh`, run by the `go` job, synthesizes the proxy the
`go/<module>/v0.1.0` tags will produce, builds every module in isolation, `go install`s the CLI into
