...
int main() {
    ...
    std::cout << [](std::string s){std::transform(s.begin(),s.end(),s.begin(),::toupper);return s;}("abc"s) << std::endl;
    std::cout << [](std::string s){std::transform(s.begin(),s.end(),s.begin(),::tolower);return s;}("XYZ"s) << std::endl;
    std::cout << ball_to_string(static_cast<int64_t>("padme"s.size())) << std::endl;
    std::cout << (("a"s + "b"s) + "c"s) << std::endl;
    return 0;
}
