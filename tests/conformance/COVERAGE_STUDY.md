...
4. **Rust's two halves are both required, and the working one's anchor must be
   ...
   **kept**, so the rule can only ever leave the denominator too large.
   ...
   - **And the per-file list is diffed, which is what sees a PARTIAL drop**
     (#676). A total collapse is the only shape arithmetic can name: 34 → 31,
     with three files moving back into the denominator, has the same signature
     as "that pin's test suite shrank by three while its library grew by three",
     so every ratio floor and the check above pass it. Any cause of a partial
     loss was therefore invisible — a path-rule regression on a language whose
     exclusions are by path (Go 13, TypeScript 6), or a reachability bug that
     drops some files rather than all of them.

     So the exclusions are committed as PATHS, not only as a count.
     `tools/coverage-study/excluded.json` is that list — language → pin → sorted
     relative paths, generated by `coverage_table.py --write` from the artifacts
     and never hand-written — and the rule needs no inference: **a path the
     committed list excludes and a run SCORED is a breach naming the file.** A
     path that is merely gone is not a breach (the pin may have deleted it);
     only positive evidence fires, the same rule the count check follows. A
     newly excluded path is reported by name and the list is regenerated, the
     same way `baseline.json` is raised, and the publish job commits it on main
     alongside the baseline.

     Three absences would disarm that diff, so each is an error rather than a
     default: a Tier A artifact that carries the count and no per-file list, a
     count that disagrees with the list beside it, and a Tier A row with no
     entry in the committed list (a row that excludes nothing declares an empty
     object; an absent key says nothing at all). `coverage_table_self_test.py`
     16a–16g assert all of it — the breach on three of thirty-four with **every
     total held fixed**, its negative control (the same totals, the list
     unchanged), and the three fail-loud shapes.

     **When a pin's test population legitimately changes** — a package moves a
     file between its library and its own test suite, or the pin list changes —
     the measuring run regenerates `excluded.json`; commit that regeneration in
     the **same PR** as the change, where a reviewer sees which files moved. Case
...
* **Rust and Go stop at stage 1.** Every scored file is an `encode-error`; the
  ...
  *next* independent gap. **Expect that of any single closed category here** —
  ...
  the compiler's own `panic!` (a construct its own encoder refuses, #632's
  class) and one reaches stage 4. `clean` is still 0 — the point above stands:
...
* `tools/coverage-study/rq1_tierb.dart` — **per-file isolation.** One
  substitution at a time, each in its own private copy of the checkout, so runs
  never compound. Answers "can this one file survive the round trip?"
* `tools/coverage-study/rq1_tierb_all.dart` — **whole-package.** Every eligible
  file substituted at once into one copy, one `dart test`, one verdict per
  package. The stricter signal: a pipeline can be right on most files and
...
### Isolation — the harness's first obligation (issue #653)

`dart test` builds the WHOLE package. A build error anywhere fails the build, so
a verdict is only about the file it names if **nothing else in that tree differs
from the pristine checkout while the suite runs**. Restoring what the harness
itself wrote is not enough for that: a checkout is an ordinary directory, and a
second Tier B process pointed at the same `--checkouts`, a diagnostic script, or
a re-clone can have a different file substituted at the same moment. Tier B
would then charge the stranger's file to whichever file it is holding, and the
row is indistinguishable from a real regression.

That is not hypothetical. `collection` measured `12/23` against CI's `20/23` on
the same pin with no encoder or compiler change in between, and 7 of the 8 false
`behavioral-drift` rows carried

```
Failed to build test:test: lib/src/list_extensions.dart:114:7:
Error: Conflicts with the implicit setter of the field 'length'.
```

for files that are **not** `list_extensions.dart`. Line 114 column 7 of the
*pristine* file is an argument list inside `lowerBoundBy`; of the **compiled-back**
file it is `set length(int input)`. The error belonged to a substituted file that
the harness was not scoring at that moment.

So the harness never writes the package's source at all:

* **Every candidate is substituted into its own private copy of the checkout**
  (`withSubstitutedCopy`), scored there, and the copy is deleted. The checkout
  the harness was pointed at stays byte-for-byte as it was found — proven by
  hash after every mode, and by the self-test's 25 ms poller *during* the run.
* **`.dart_tool/test/` is the one subtree never copied.** It is `dart test`'s own
  build cache, and a kernel is keyed by `package:` URI, so carrying candidate
  N-1's compiled kernel into candidate N would survive the change of directory
  unnoticed. Every candidate therefore starts from the same baseline state
  rather than from its predecessor's — the harness no longer depends on that
  cache invalidating correctly after a substitute-and-restore, an assumption it
  used to make silently and that cannot be verified from outside the CFE.
* **A checkout that changes underneath a run FAILS LOUD.** The tree is
  snapshotted by SHA-256 after the baseline and re-verified before every
  candidate (`verifyTreeUnchanged`); a mismatch **throws** instead of scoring,
  because from the first changed byte the baseline no longer describes the tree
  being measured. Isolation makes a run harmless to others; it cannot make
  others harmless to it, since the copy is taken from the shared checkout.

Two properties fall out for free: a crashed run cannot leave a checkout dirty,
and two runs pointed at the same checkout can only ever stop each other loudly,
never corrupt each other quietly. The CI job still re-asserts cleanliness from
...
real pipeline: the checkout is left byte-for-byte as it was found; a file whose
...
It also carries the **negative control for #653**: a second scratch package of
two files where `lib/aa_alpha.dart`'s substitute does not build and
`lib/bb_beta.dart`'s substitute is BYTE-IDENTICAL to its own source. `bb_beta`
cannot change behaviour, so `clean` is the only honest verdict for it, while
`aa_alpha` must stay `behavioral-drift` — the instrument has to keep its TRUE
red. Those substitutions are injected rather than encoded on purpose: a negative
control has to be unfalsifiable by the next encoder change. Alongside them a
poller hashes every `lib/` file every 25 ms for the whole run and fails on a
single differing byte, and two rows cover `verifyTreeUnchanged` — an unchanged
tree passes, a tree changed under the run throws. The poller is the row that
fails on the pre-#653 in-place harness.
...
# --excluded-list is the committed per-file test-only exclusion list (#676),
# and it is REQUIRED: without it a readmitted exclusion has nothing to be
# diffed against, so the check would pass vacuously.

# Add --write to regenerate the README block and persist any raised floor and
# any change to the excluded list (what the `publish` job runs), and
# --summary "$GITHUB_STEP_SUMMARY" to also print the table into the job
# summary.
```
