...
int main() {
    auto count = BallDyn(0LL);
    for (auto i = 1LL; (i <= 10LL); ball_assign(i, (i + 1LL))) {
        if (([&](int64_t _a, int64_t _b){ auto _r = _a % _b; return _r < 0 ? _r + (_b < 0 ? -_b : _b) : _r; }(static_cast<int64_t>(i), static_cast<int64_t>(2LL)) == 0LL)) {
            continue;
        }
        if ((i == 7LL)) {
            break;
        }
        ball_assign(count, (count + 1LL));
