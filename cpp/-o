...
// Source: 207_record_pattern_destructure v1.0.0
...
BallDyn describe() {
    auto point = BallDyn(std::unordered_map<std::string, std::any>{{"x", std::any(10LL)}, {"y", std::any(20LL)}});
    return [&]() -> BallDyn {
      auto __subj = BallDyn(point);
      if (true) return [&](BallDyn a, BallDyn b) -> BallDyn { return ((ball_to_string(a) + ","s) + ball_to_string(b)); }(BallDyn(static_cast<BallDyn>(BallDyn(__subj))["x"s]), BallDyn(static_cast<BallDyn>(BallDyn(__subj))["y"s]));
      return BallDyn();
    }();
    return BallDyn();
}
...
int main() {
    std::cout << ball_to_string(describe()) << std::endl;
    return 0;
}
