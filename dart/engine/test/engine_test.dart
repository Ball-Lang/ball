    ...
    'types': [
      {
        'name': 'PrintInput',
        'field': [
          {'name': 'message', 'number': 1, 'label': 'LABEL_OPTIONAL', 'type': 'TYPE_STRING'},
        ],
      },
      {
        'name': 'BinaryInput',
        'field': [
          {'name': 'left', 'number': 1, 'label': 'LABEL_OPTIONAL', 'type': 'TYPE_INT64'},
          {'name': 'right', 'number': 2, 'label': 'LABEL_OPTIONAL', 'type': 'TYPE_INT64'},
        ],
      },
      {
        'name': 'UnaryInput',
        'field': [
          {'name': 'value', 'number': 1, 'label': 'LABEL_OPTIONAL', 'type': 'TYPE_STRING'},
        ],
      },
    ],
    'functions': [
...
Map<String, dynamic> literal(Object value) {
  if (value is int) return {'literal': {'intValue': '$value'}};
  if (value is double) return {'literal': {'doubleValue': value}};
  if (value is String) return {'literal': {'stringValue': value}};
  if (value is bool) return {'literal': {'boolValue': value}};
  throw ArgumentError('Unsupported literal type: ${value.runtimeType}');
}

Map<String, dynamic> ref(String name) => {'reference': {'name': name}};

Map<String, dynamic> call(String function,
    {String module = '', Map<String, dynamic>? input}) {
...
Map<String, dynamic> printExpr(Map<String, dynamic> value) {
  return stdCall('print', msg([field('message', value)], typeName: 'PrintInput'));
}
...
Map<String, dynamic> functionDef(
  ...
  if (params.isNotEmpty) {
    fn['metadata'] = {
      'kind': 'function',
      'params': params.map((p) => {'name': p['name'], 'type': p['type']}).toList(),
    };
  }
  return fn;
}
...
Map<String, dynamic> listLit(List<Map<String, dynamic>> elements) =>
    {'literal': {'listValue': {'elements': elements}}};
...
Map<String, dynamic> indexExpr(
    Map<String, dynamic> target, Map<String, dynamic> idx) =>
...
void main() {
  ...
  group('all_constructs.ball.json', () {
    ...
    setUpAll(() {
      program = loadProgram('../../examples/all_constructs/all_constructs.ball.json');
    ...
    test('local vars and nested calls', () {
      ...
      expect(lines[32], '14');   // nested(5) = add(multiply(5,2), subtract(5,1)) = 10+4 = 14
  ...
  group('engine: literals and print', () {
    test('prints a string literal', () {
      final program = buildProgram(functions: [
  ...
  group('engine: arithmetic', () {
    test('add two integers', () {
      final program = buildProgram(functions: [
        mainFn([
          stmt(printToString(
            stdCall('add', msg([field('left', literal(10)), field('right', literal(20))])),
          )),
        ]),
      ]);
    ...
    test('subtract', () {
      final program = buildProgram(functions: [
        mainFn([
          stmt(printToString(
            stdCall('subtract', msg([field('left', literal(100)), field('right', literal(37))])),
          )),
        ]),
      ]);
    ...
    test('multiply', () {
      final program = buildProgram(functions: [
        mainFn([
          stmt(printToString(
            stdCall('multiply', msg([field('left', literal(7)), field('right', literal(8))])),
          )),
        ]),
      ]);
  ...
  group('engine: comparisons', () {
    test('less_than true', () {
      final program = buildProgram(functions: [
        mainFn([
          stmt(printToString(
            stdCall('less_than', msg([field('left', literal(1)), field('right', literal(2))])),
          )),
        ]),
      ]);
      expect(runAndCapture(program), ['true']);
    ...
    test('less_than false', () {
      final program = buildProgram(functions: [
        mainFn([
          stmt(printToString(
            stdCall('less_than', msg([field('left', literal(5)), field('right', literal(3))])),
          )),
        ]),
      ]);
    ...
    test('equals true', () {
      final program = buildProgram(functions: [
        mainFn([
          stmt(printToString(
            stdCall('equals', msg([field('left', literal(7)), field('right', literal(7))])),
          )),
        ]),
      ]);
      expect(runAndCapture(program), ['true']);
  ...
  group('engine: let bindings and variables', () {
    ...
    test('multiple let bindings', () {
      final program = buildProgram(functions: [
        mainFn([
          ...
          stmt(printToString(
            stdCall('add', msg([field('left', ref('a')), field('right', ref('b'))])),
          )),
        ]),
      ]);
      expect(runAndCapture(program), ['30']);
    ...
    test('let binding uses expression', () {
      final program = buildProgram(functions: [
        mainFn([
          letStmt('sum', stdCall('add', msg([field('left', literal(3)), field('right', literal(4))]))),
  ...
  group('engine: user-defined functions', () {
    test('single-parameter function', () {
      ...
      final program = buildProgram(functions: [
        functionDef(
          ...
          body: stdCall('add', msg([field('left', ref('n')), field('right', ref('n'))])),
        ),
        mainFn([
          stmt(printToString(call('double_it', input: literal(21)))),
        ]),
      ]);
      expect(runAndCapture(program), ['42']);
    });
    ...
    test('two-parameter function with arg0/arg1', () {
      ...
      final program = buildProgram(functions: [
        functionDef(
          ...
          body: stdCall('add', msg([field('left', ref('a')), field('right', ref('b'))])),
        ),
        mainFn([
          stmt(printToString(
            call('myAdd', input: msg([field('arg0', literal(3)), field('arg1', literal(4))])),
          )),
        ]),
      ]);
      expect(runAndCapture(program), ['7']);
    ...
    test('recursive function (fibonacci)', () {
      ...
      final program = buildProgram(functions: [
        functionDef(
          ...
          body: {
            'block': {
              'statements': [
                stmt(stdCall('if', msg([
                  field('condition', stdCall('lte', msg([field('left', ref('n')), field('right', literal(1))]))),
                  field('then', {'block': {'statements': [
                    stmt(stdCall('return', msg([field('value', ref('n'))]))),
                  ]}}),
                  field('else', {'block': {'statements': [
                    stmt(stdCall('return', msg([field('value',
                      stdCall('add', msg([
                        field('left', call('fib', input: stdCall('subtract', msg([field('left', ref('n')), field('right', literal(1))])))),
                        field('right', call('fib', input: stdCall('subtract', msg([field('left', ref('n')), field('right', literal(2))])))),
                      ])),
                    )]))),
                  ]}}),
                ]))),
        ...
        mainFn([
          stmt(printToString(call('fib', input: literal(10)))),
        ]),
      ]);
      ...
      final program = buildProgram(functions: [
        mainFn([
          stmt(stdCall('if', msg([
            ...
            field('then', {'block': {'statements': [stmt(printStr('yes'))]}}),
            field('else', {'block': {'statements': [stmt(printStr('no'))]}}),
          ]))),
        ]),
      ]);
    ...
    test('if without else', () {
      final program = buildProgram(functions: [
        mainFn([
          stmt(stdCall('if', msg([
            field('condition', literal(true)),
            field('then', {'block': {'statements': [stmt(printStr('only if'))]}}),
          ]))),
        ]),
      ]);
    ...
    test('nested if', () {
      final program = buildProgram(functions: [
        mainFn([
          letStmt('x', literal(5)),
          stmt(stdCall('if', msg([
            field('condition', stdCall('greater_than', msg([field('left', ref('x')), field('right', literal(10))]))),
            field('then', {'block': {'statements': [stmt(printStr('big'))]}}),
            field('else', {'block': {'statements': [
              stmt(stdCall('if', msg([
                field('condition', stdCall('greater_than', msg([field('left', ref('x')), field('right', literal(0))]))),
                field('then', {'block': {'statements': [stmt(printStr('small positive'))]}}),
                field('else', {'block': {'statements': [stmt(printStr('non-positive'))]}}),
              ]))),
            ]}}),
          ]))),
        ]),
  ...
  group('engine: top-level variables', () {
    test('const top-level variable', () {
      final program = buildProgram(functions: [
        ...
        mainFn([
          stmt(printToString(ref('myConst'))),
        ]),
      ]);
    ...
    test('multiple top-level variables', () {
      final program = buildProgram(functions: [
        ...
        mainFn([
          stmt(printToString(
            stdCall('add', msg([field('left', ref('x')), field('right', ref('y'))])),
          )),
        ]),
      ]);
      expect(runAndCapture(program), ['30']);
    ...
    test('while loop with counter', () {
      final program = buildProgram(functions: [
        mainFn([
          ...
          stmt(stdCall('while', msg([
            field('condition', stdCall('less_than', msg([field('left', ref('i')), field('right', literal(3))]))),
            field('body', {'block': {'statements': [
              stmt(printToString(ref('i'))),
              stmt(stdCall('assign', msg([field('target', ref('i')), field('value',
                stdCall('add', msg([field('left', ref('i')), field('right', literal(1))])),
              )]))),
            ]}}),
          ]))),
        ]),
  ...
  group('engine: for loop', () {
    test('for loop counting', () {
      ...
      final program = buildProgram(functions: [
        mainFn([
          letStmt('i', literal(0), keyword: 'var'),
          stmt(stdCall('for', msg([
            field('init', literal('var i = 0')),
            field('condition', stdCall('less_than', msg([field('left', ref('i')), field('right', literal(3))]))),
            field('update', stdCall('assign', msg([field('target', ref('i')), field('value',
              stdCall('add', msg([field('left', ref('i')), field('right', literal(1))])),
            )]))),
            field('body', {'block': {'statements': [
              stmt(printToString(ref('i'))),
  ...
  group('engine: string operations', () {
    test('string concatenation via add', () {
      final program = buildProgram(functions: [
        mainFn([
          stmt(printExpr(
            stdCall('add', msg([field('left', literal('Hello, ')), field('right', literal('World!'))])),
          )),
        ]),
      ]);
  ...
  group('engine: error handling', () {
    test('throws on undefined variable', () {
      final program = buildProgram(functions: [
        mainFn([
          stmt(printToString(ref('undefined_var'))),
        ]),
      ]);
      expect(
  ...
  group('engine: protobuf deserialization', () {
    test('Program round-trips through proto3 JSON', () {
      final program = loadProgram('../../examples/hello_world/hello_world.ball.json');
    ...
    test('Program round-trips through binary protobuf', () {
      final program = loadProgram('../../examples/fibonacci/fibonacci.ball.json');
      ...
      final program = buildProgram(functions: [
        mainFn([
          ...
          stmt(printExpr(stdCall('string_interpolation', msg([
            field('parts', listLit([literal('Hello, '), ref('name'), literal('!')])),
          ])))),
        ]),
      ]);
      expect(runAndCapture(program), ['Hello, World!']);
  ...
  group('engine: field access — virtual properties on List', () {
    test('.length', () {
      final program = buildProgram(functions: [
        mainFn([stmt(printToString(fieldAcc(listLit([literal(1), literal(2), literal(3)]), 'length')))]),
      ]);
    ...
    test('.isNotEmpty on non-empty list', () {
      final program = buildProgram(functions: [
        mainFn([stmt(printToString(fieldAcc(listLit([literal(1)]), 'isNotEmpty')))]),
      ]);
      expect(runAndCapture(program), ['true']);
    });
    ...
    test('.first', () {
      final program = buildProgram(functions: [
        mainFn([stmt(printToString(fieldAcc(listLit([literal(10), literal(20), literal(30)]), 'first')))]),
      ]);
    ...
    test('.last', () {
      final program = buildProgram(functions: [
        mainFn([stmt(printToString(fieldAcc(listLit([literal(10), literal(20), literal(30)]), 'last')))]),
      ]);
      expect(runAndCapture(program), ['30']);
    });
    ...
    test('.reversed', () {
      final program = buildProgram(functions: [
        mainFn([
          letStmt('rev', fieldAcc(listLit([literal(1), literal(2), literal(3)]), 'reversed')),
  ...
  group('engine: field access — message fields', () {
    test('access typed message fields', () {
      final program = buildProgram(functions: [
        mainFn([
          letStmt('p', msg([field('x', literal(10)), field('y', literal(20))], typeName: 'Point')),
  ...
  group('engine: bitwise operations', () {
    ...
    test('bitwise AND', () {
      final p = buildProgram(stdFunctions: bw, functions: [
        mainFn([stmt(printToString(stdCall('bitwise_and', msg([field('left', literal(6)), field('right', literal(3))]))))]),
      ]);
    ...
    test('bitwise OR', () {
      final p = buildProgram(stdFunctions: bw, functions: [
        mainFn([stmt(printToString(stdCall('bitwise_or', msg([field('left', literal(6)), field('right', literal(3))]))))]),
      ]);
    ...
    test('bitwise XOR', () {
      final p = buildProgram(stdFunctions: bw, functions: [
        mainFn([stmt(printToString(stdCall('bitwise_xor', msg([field('left', literal(6)), field('right', literal(3))]))))]),
      ]);
    ...
    test('bitwise NOT', () {
      final p = buildProgram(stdFunctions: bw, functions: [
        mainFn([stmt(printToString(stdCall('bitwise_not', msg([field('value', literal(0))]))))]),
      ]);
    ...
    test('left shift', () {
      final p = buildProgram(stdFunctions: bw, functions: [
        mainFn([stmt(printToString(stdCall('left_shift', msg([field('left', literal(1)), field('right', literal(3))]))))]),
      ]);
    ...
    test('right shift', () {
      final p = buildProgram(stdFunctions: bw, functions: [
        mainFn([stmt(printToString(stdCall('right_shift', msg([field('left', literal(8)), field('right', literal(2))]))))]),
      ]);
      expect(runAndCapture(p), ['2']);
    });
  });
  ...
  group('engine: string operations', () {
    ...
    test('string_length', () {
      final p = buildProgram(stdFunctions: strFns, functions: [
        mainFn([stmt(printToString(stdCall('string_length', msg([field('value', literal('hello'))]))))]),
      ]);
      expect(runAndCapture(p), ['5']);
    });
    ...
    test('string_is_empty — true', () {
      final p = buildProgram(stdFunctions: strFns, functions: [
        mainFn([stmt(printToString(stdCall('string_is_empty', msg([field('value', literal(''))]))))]),
      ]);
    ...
    test('string_is_empty — false', () {
      final p = buildProgram(stdFunctions: strFns, functions: [
        mainFn([stmt(printToString(stdCall('string_is_empty', msg([field('value', literal('x'))]))))]),
      ]);
    ...
    test('string_to_upper', () {
      final p = buildProgram(stdFunctions: strFns, functions: [
        mainFn([stmt(printExpr(stdCall('string_to_upper', msg([field('value', literal('hello'))]))))]),
      ]);
    ...
    test('string_to_lower', () {
      final p = buildProgram(stdFunctions: strFns, functions: [
        mainFn([stmt(printExpr(stdCall('string_to_lower', msg([field('value', literal('WORLD'))]))))]),
      ]);
    ...
    test('string_trim', () {
      final p = buildProgram(stdFunctions: strFns, functions: [
        mainFn([stmt(printExpr(stdCall('string_trim', msg([field('value', literal('  hi  '))]))))]),
      ]);
    ...
    test('string_trim_start', () {
      final p = buildProgram(stdFunctions: strFns, functions: [
        mainFn([stmt(printExpr(stdCall('string_trim_start', msg([field('value', literal('  hi  '))]))))]),
      ]);
      expect(runAndCapture(p), ['hi  ']);
    });
    ...
    test('string_trim_end', () {
      final p = buildProgram(stdFunctions: strFns, functions: [
        mainFn([stmt(printExpr(stdCall('string_trim_end', msg([field('value', literal('  hi  '))]))))]),
      ]);
      expect(runAndCapture(p), ['  hi']);
    });
    ...
    test('string_contains — true', () {
      final p = buildProgram(stdFunctions: strFns, functions: [
        mainFn([stmt(printToString(stdCall('string_contains', msg([field('left', literal('hello world')), field('right', literal('world'))]))))]),
      ]);
      expect(runAndCapture(p), ['true']);
    });
    ...
    test('string_starts_with — true', () {
      final p = buildProgram(stdFunctions: strFns, functions: [
        mainFn([stmt(printToString(stdCall('string_starts_with', msg([field('left', literal('hello')), field('right', literal('hel'))]))))]),
      ]);
      expect(runAndCapture(p), ['true']);
    });
    ...
    test('string_ends_with — true', () {
      final p = buildProgram(stdFunctions: strFns, functions: [
        mainFn([stmt(printToString(stdCall('string_ends_with', msg([field('left', literal('hello')), field('right', literal('llo'))]))))]),
      ]);
      expect(runAndCapture(p), ['true']);
    });
    ...
    test('string_substring', () {
      final p = buildProgram(stdFunctions: strFns, functions: [
        mainFn([stmt(printExpr(stdCall('string_substring', msg([
    ...
    test('string_split', () {
      final splitFns = [...strFns, {'name': 'index', 'isBase': true}];
      final p = buildProgram(stdFunctions: splitFns, functions: [
        mainFn([
          letStmt('parts', stdCall('string_split', msg([field('left', literal('a,b,c')), field('right', literal(','))]))),
    ...
    test('string_replace', () {
      final p = buildProgram(stdFunctions: strFns, functions: [
        mainFn([stmt(printExpr(stdCall('string_replace', msg([
    ...
    test('string_replace_all', () {
      final p = buildProgram(stdFunctions: strFns, functions: [
        mainFn([stmt(printExpr(stdCall('string_replace_all', msg([
    ...
    test('string_repeat', () {
      final p = buildProgram(stdFunctions: strFns, functions: [
        mainFn([stmt(printExpr(stdCall('string_repeat', msg([
    ...
    test('string_pad_left', () {
      final p = buildProgram(stdFunctions: strFns, functions: [
        mainFn([stmt(printExpr(stdCall('string_pad_left', msg([
    ...
    test('string_pad_right', () {
      final p = buildProgram(stdFunctions: strFns, functions: [
        mainFn([stmt(printExpr(stdCall('string_pad_right', msg([
    ...
    test('string_char_at', () {
      final p = buildProgram(stdFunctions: strFns, functions: [
        mainFn([stmt(printExpr(stdCall('string_char_at', msg([
    ...
    test('string_char_code_at + string_from_char_code round-trip', () {
      final p = buildProgram(stdFunctions: strFns, functions: [
        mainFn([
          letStmt('code', stdCall('string_char_code_at', msg([field('target', literal('A')), field('index', literal(0))]))),
          stmt(printExpr(stdCall('string_from_char_code', msg([field('value', ref('code'))])))),
        ]),
      ]);
    ...
    test('string_index_of', () {
      final p = buildProgram(stdFunctions: strFns, functions: [
        mainFn([stmt(printToString(stdCall('string_index_of', msg([field('left', literal('hello')), field('right', literal('ll'))]))))]),
      ]);
      expect(runAndCapture(p), ['2']);
    });
  });
  ...
  group('engine: math operations', () {
    ...
    test('math_abs', () {
      final p = buildProgram(stdFunctions: mathFns, functions: [
        mainFn([stmt(printToString(stdCall('math_abs', msg([field('value', literal(-7))]))))]),
      ]);
      expect(runAndCapture(p), ['7']);
    });
    ...
    test('math_floor', () {
      final p = buildProgram(stdFunctions: mathFns, functions: [
        mainFn([stmt(printToString(stdCall('math_floor', msg([field('value', literal(3.9))]))))]),
      ]);
    ...
    test('math_ceil', () {
      final p = buildProgram(stdFunctions: mathFns, functions: [
        mainFn([stmt(printToString(stdCall('math_ceil', msg([field('value', literal(3.1))]))))]),
      ]);
    ...
    test('math_round', () {
      final p = buildProgram(stdFunctions: mathFns, functions: [
        mainFn([stmt(printToString(stdCall('math_round', msg([field('value', literal(3.5))]))))]),
      ]);
      expect(runAndCapture(p), ['4']);
    });
    ...
    test('math_sqrt of 25', () {
      final p = buildProgram(stdFunctions: mathFns, functions: [
        mainFn([stmt(printToString(stdCall('math_sqrt', msg([field('value', literal(25.0))]))))]),
      ]);
    ...
    test('math_pow', () {
      final p = buildProgram(stdFunctions: mathFns, functions: [
        mainFn([stmt(printToString(stdCall('math_pow', msg([field('left', literal(2.0)), field('right', literal(10.0))]))))]),
      ]);
    ...
    test('math_min', () {
      final p = buildProgram(stdFunctions: mathFns, functions: [
        mainFn([stmt(printToString(stdCall('math_min', msg([field('left', literal(3)), field('right', literal(7))]))))]),
      ]);
      expect(runAndCapture(p), ['3']);
    });
    ...
    test('math_max', () {
      final p = buildProgram(stdFunctions: mathFns, functions: [
        mainFn([stmt(printToString(stdCall('math_max', msg([field('left', literal(3)), field('right', literal(7))]))))]),
      ]);
      expect(runAndCapture(p), ['7']);
    });
    ...
    test('math_clamp', () {
      final p = buildProgram(stdFunctions: mathFns, functions: [
        mainFn([stmt(printToString(stdCall('math_clamp', msg([
    ...
    test('math_pi is approximately 3.14', () {
      final p = buildProgram(stdFunctions: mathFns, functions: [
        mainFn([
          ...
          stmt(printToString(stdCall('greater_than', msg([field('left', ref('pi')), field('right', literal(3.14))])))),
        ]),
      ]);
      expect(runAndCapture(p), ['true']);
    });
    ...
    test('math_infinity', () {
      final p = buildProgram(stdFunctions: mathFns, functions: [
        mainFn([stmt(printToString(stdCall('math_is_infinite', msg([
          field('value', stdCall('math_infinity', msg([]))),
        ]))))]),
      ]);
      expect(runAndCapture(p), ['true']);
    });
    ...
    test('math_nan', () {
      final p = buildProgram(stdFunctions: mathFns, functions: [
        mainFn([stmt(printToString(stdCall('math_is_nan', msg([
          field('value', stdCall('math_nan', msg([]))),
        ]))))]),
      ]);
      expect(runAndCapture(p), ['true']);
    });
    ...
    test('math_gcd', () {
      final p = buildProgram(stdFunctions: mathFns, functions: [
        mainFn([stmt(printToString(stdCall('math_gcd', msg([field('left', literal(12)), field('right', literal(8))]))))]),
      ]);
      expect(runAndCapture(p), ['4']);
    });
    ...
    test('math_sign positive', () {
      final p = buildProgram(stdFunctions: mathFns, functions: [
        mainFn([stmt(printToString(stdCall('math_sign', msg([field('value', literal(5))]))))]),
      ]);
      expect(runAndCapture(p), ['1']);
    });
    ...
    test('math_sign negative', () {
      final p = buildProgram(stdFunctions: mathFns, functions: [
        mainFn([stmt(printToString(stdCall('math_sign', msg([field('value', literal(-3))]))))]),
      ]);
      expect(runAndCapture(p), ['-1']);
    });
  });
  ...
  group('engine: for_in loop', () {
    test('iterates over a list', () {
      final program = buildProgram(functions: [
        mainFn([
          ...
          stmt(stdCall('for_in', msg([
            field('variable', literal('item')),
            field('iterable', ref('nums')),
            field('body', {'block': {'statements': [stmt(printToString(ref('item')))]}}),
          ]))),
        ]),
      ]);
    ...
    test('break inside for_in', () {
      final program = buildProgram(functions: [
        mainFn([
          letStmt('nums', listLit([literal(1), literal(2), literal(3), literal(4)])),
          stmt(stdCall('for_in', msg([
            field('variable', literal('x')),
            field('iterable', ref('nums')),
            field('body', {'block': {'statements': [
              stmt(stdCall('if', msg([
                field('condition', stdCall('equals', msg([field('left', ref('x')), field('right', literal(3))]))),
                field('then', {'block': {'statements': [stmt(stdCall('break', msg([])))]}}),
              ]))),
              stmt(printToString(ref('x'))),
            ]}}),
          ]))),
      ...
      final program = buildProgram(functions: [
        mainFn([
          ...
          stmt(stdCall('for_in', msg([
            ...
            field('iterable', listLit([literal(1), literal(2), literal(3), literal(4), literal(5)])),
            field('body', {'block': {'statements': [
              stmt(stdCall('assign', msg([
                field('target', ref('total')),
                field('value', stdCall('add', msg([field('left', ref('total')), field('right', ref('n'))]))),
              ]))),
            ]}}),
          ]))),
  ...
  group('engine: do_while loop', () {
    test('runs body at least once', () {
      final program = buildProgram(functions: [
        mainFn([
          letStmt('i', literal(0), keyword: 'var'),
          stmt(stdCall('do_while', msg([
            field('body', {'block': {'statements': [
              stmt(printToString(ref('i'))),
              stmt(stdCall('assign', msg([
                field('target', ref('i')),
                field('value', stdCall('add', msg([field('left', ref('i')), field('right', literal(1))]))),
              ]))),
            ]}}),
            field('condition', stdCall('less_than', msg([field('left', ref('i')), field('right', literal(3))]))),
          ]))),
    ...
    test('runs body once even if condition false', () {
      final program = buildProgram(functions: [
        mainFn([
          stmt(stdCall('do_while', msg([
            field('body', {'block': {'statements': [stmt(printStr('once'))]}}),
            field('condition', literal(false)),
          ]))),
        ]),
      ]);
  ...
  group('engine: switch statement', () {
    test('matches a case', () {
      final program = buildProgram(functions: [
        mainFn([
          letStmt('x', literal(2)),
          stmt(stdCall('switch', msg([
            field('subject', ref('x')),
            field('cases', listLit([
              msg([field('value', literal(1)), field('body', {'block': {'statements': [stmt(printStr('one'))]}})])  ,
              msg([field('value', literal(2)), field('body', {'block': {'statements': [stmt(printStr('two'))]}})])  ,
              msg([field('value', literal(3)), field('body', {'block': {'statements': [stmt(printStr('three'))]}})])  ,
            ])),
          ]))),
        ]),
      ]);
      ...
      final program = buildProgram(functions: [
        mainFn([
          ...
          stmt(stdCall('switch', msg([
            ...
            field('cases', listLit([
              msg([field('value', literal(1)), field('body', {'block': {'statements': [stmt(printStr('one'))]}})])  ,
              msg([field('is_default', literal(true)), field('body', {'block': {'statements': [stmt(printStr('default'))]}})])  ,
            ])),
          ]))),
        ]),
      ]);
  ...
  group('engine: switch expression', () {
    final switchFn = [{'name': 'switch_expr', 'isBase': true}];

    test('matches string pattern', () {
      final p = buildProgram(stdFunctions: switchFn, functions: [
        mainFn([
          letStmt('x', literal(2)),
          letStmt('r', stdCall('switch_expr', msg([
            field('subject', ref('x')),
            field('cases', listLit([
              msg([field('pattern', literal('1')), field('body', literal('one'))]),
              msg([field('pattern', literal('2')), field('body', literal('two'))]),
              msg([field('pattern', literal('_')), field('body', literal('other'))]),
            ])),
          ]))),
      ...
      final p = buildProgram(stdFunctions: switchFn, functions: [
        mainFn([
          ...
          letStmt('r', stdCall('switch_expr', msg([
            ...
            field('cases', listLit([
              msg([field('pattern', literal('1')), field('body', literal('one'))]),
              msg([field('pattern', literal('_')), field('body', literal('default'))]),
            ])),
          ]))),
          stmt(printExpr(ref('r'))),
        ]),
      ...
      final p = buildProgram(stdFunctions: switchFn, functions: [
        mainFn([
          ...
          letStmt('r', stdCall('switch_expr', msg([
            field('subject', ref('x')),
            field('cases', listLit([
              msg([field('pattern', literal('String')), field('body', literal('is string'))]),
              msg([field('pattern', literal('int')), field('body', literal('is int'))]),
              msg([field('pattern', literal('_')), field('body', literal('other'))]),
            ])),
          ]))),
          stmt(printExpr(ref('r'))),
      ...
      final p = buildProgram(stdFunctions: switchFn, functions: [
        mainFn([
          ...
          letStmt('r', stdCall('switch_expr', msg([
            field('subject', ref('x')),
            field('cases', listLit([
              msg([field('pattern', literal(7)), field('body', literal('seven'))]),
              msg([field('pattern', literal('_')), field('body', literal('other'))]),
            ])),
          ]))),
          stmt(printExpr(ref('r'))),
  ...
  group('engine: try/catch/finally', () {
    test('finally runs even without throw', () {
      final program = buildProgram(functions: [
        mainFn([
          stmt(stdCall('try', msg([
            field('body', {'block': {'statements': [stmt(printStr('try'))]}}),
            field('finally', {'block': {'statements': [stmt(printStr('finally'))]}}),
          ]))),
        ]),
      ]);
    ...
    test('catch handles thrown exception', () {
      final throwFns = [{'name': 'throw', 'isBase': true}];
      final program = buildProgram(stdFunctions: throwFns, functions: [
        mainFn([
          stmt(stdCall('try', msg([
            field('body', {'block': {'statements': [
              stmt(stdCall('throw', msg([field('value', literal('boom'))]))),
            ]}}),
            field('catches', listLit([
              msg([
                field('variable', literal('e')),
                field('body', {'block': {'statements': [stmt(printStr('caught'))]}}),
              ]),
            ])),
          ]))),
          stmt(printStr('after')),
    ...
    test('catch + finally', () {
      ...
      final program = buildProgram(stdFunctions: throwFns, functions: [
        mainFn([
          stmt(stdCall('try', msg([
            field('body', {'block': {'statements': [
              stmt(stdCall('throw', msg([field('value', literal('err'))]))),
            ]}}),
            field('catches', listLit([
              msg([field('variable', literal('e')), field('body', {'block': {'statements': [stmt(printStr('caught'))]}})])  ,
            ])),
            field('finally', {'block': {'statements': [stmt(printStr('finally'))]}}),
          ]))),
        ]),
    ...
    test('break exits for loop early', () {
      final program = buildProgram(functions: [
        mainFn([
          ...
          stmt(stdCall('for', msg([
            field('init', literal('var i = 0')),
            field('condition', stdCall('less_than', msg([field('left', ref('i')), field('right', literal(10))]))),
            field('update', stdCall('assign', msg([
              field('target', ref('i')),
              field('value', stdCall('add', msg([field('left', ref('i')), field('right', literal(1))]))),
            ]))),
            field('body', {'block': {'statements': [
              stmt(printToString(ref('i'))),
              stmt(stdCall('if', msg([
                field('condition', stdCall('equals', msg([field('left', ref('i')), field('right', literal(2))]))),
                field('then', {'block': {'statements': [stmt(stdCall('break', msg([])))]}}),
              ]))),
            ]}}),
          ]))),
    ...
    test('continue skips iteration', () {
      final program = buildProgram(functions: [
        mainFn([
          ...
          stmt(stdCall('for', msg([
            field('init', literal('var i = 0')),
            field('condition', stdCall('less_than', msg([field('left', ref('i')), field('right', literal(5))]))),
            field('update', stdCall('assign', msg([
              field('target', ref('i')),
              field('value', stdCall('add', msg([field('left', ref('i')), field('right', literal(1))]))),
            ]))),
            field('body', {'block': {'statements': [
              stmt(stdCall('if', msg([
                field('condition', stdCall('equals', msg([field('left', ref('i')), field('right', literal(2))]))),
                field('then', {'block': {'statements': [stmt(stdCall('continue', msg([])))]}}),
              ]))),
              stmt(printToString(ref('i'))),
            ]}}),
          ]))),
      ...
      final program = buildProgram(functions: [
        mainFn([
          ...
          stmt(stdCall('while', msg([
            ...
            field('body', {'block': {'statements': [
              stmt(stdCall('if', msg([
                field('condition', stdCall('equals', msg([field('left', ref('i')), field('right', literal(3))]))),
                field('then', {'block': {'statements': [stmt(stdCall('break', msg([])))]}}),
              ]))),
              stmt(printToString(ref('i'))),
              stmt(stdCall('assign', msg([
  ...
  group('engine: lambda and closures', () {
    final invokeFns = [{'name': 'invoke', 'isBase': true}];

    test('lambda with single input param', () {
      final p = buildProgram(stdFunctions: invokeFns, functions: [
        mainFn([
          letStmt('addOne', lambdaExpr(
            stdCall('add', msg([field('left', ref('input')), field('right', literal(1))])),
            inputType: 'int',
          )),
          stmt(printToString(stdCall('invoke', msg([
    ...
    test('closure captures enclosing variable', () {
      final p = buildProgram(stdFunctions: invokeFns, functions: [
        mainFn([
          ...
          letStmt('addOffset', lambdaExpr(
            stdCall('add', msg([field('left', ref('input')), field('right', ref('offset'))])),
            inputType: 'int',
          )),
          stmt(printToString(stdCall('invoke', msg([
    ...
    test('lambda with no arguments (null input)', () {
      final p = buildProgram(stdFunctions: invokeFns, functions: [
        mainFn([
          ...
          stmt(printExpr(stdCall('invoke', msg([
            field('callee', ref('greeting')),
          ])))),
        ]),
      ]);
      expect(runAndCapture(p), ['hello!']);
    ...
    test('lambda returned from function', () {
      final p = buildProgram(stdFunctions: invokeFns, functions: [
        functionDef('makeAdder',
          inputType: 'int',
          outputType: 'Function',
          params: [{'name': 'n', 'type': 'int'}],
          body: lambdaExpr(
            stdCall('add', msg([field('left', ref('input')), field('right', ref('n'))])),
            inputType: 'int',
          ),
        ),
        mainFn([
  ...
  group('engine: index operator', () {
    final indexFns = [{'name': 'index', 'isBase': true}];

    test('index into list', () {
      final p = buildProgram(stdFunctions: indexFns, functions: [
        mainFn([
          letStmt('nums', listLit([literal(10), literal(20), literal(30)])),
    ...
    test('index set (list mutation)', () {
      final p = buildProgram(stdFunctions: indexFns, functions: [
        mainFn([
          letStmt('arr', listLit([literal(1), literal(2), literal(3)]), keyword: 'var'),
          stmt(stdCall('assign', msg([
  ...
  group('engine: compound assign', () {
    test('+= operator', () {
      final program = buildProgram(functions: [
        mainFn([
          ...
          stmt(stdCall('assign', msg([field('target', ref('x')), field('value', literal(5)), field('op', literal('+='))]))),
          stmt(printToString(ref('x'))),
        ]),
      ]);
      expect(runAndCapture(program), ['15']);
    ...
    test('-= operator', () {
      final program = buildProgram(functions: [
        mainFn([
          letStmt('x', literal(10), keyword: 'var'),
          stmt(stdCall('assign', msg([field('target', ref('x')), field('value', literal(3)), field('op', literal('-='))]))),
          stmt(printToString(ref('x'))),
        ]),
      ]);
      expect(runAndCapture(program), ['7']);
    ...
    test('??= operator — assigns when null', () {
      final program = buildProgram(functions: [
        mainFn([
          ...
          stmt(stdCall('assign', msg([field('target', ref('x')), field('value', literal(42)), field('op', literal('??='))]))),
          stmt(printToString(ref('x'))),
        ]),
      ]);
      expect(runAndCapture(program), ['42']);
  ...
  group('engine: increment and decrement', () {
    test('pre_increment returns new value and mutates', () {
      final program = buildProgram(functions: [
        mainFn([
          ...
          stmt(printToString(stdCall('pre_increment', msg([field('value', ref('i'))])))),
          stmt(printToString(ref('i'))),
        ]),
      ]);
    ...
    test('post_increment returns old value but mutates', () {
      final program = buildProgram(functions: [
        mainFn([
          letStmt('i', literal(5), keyword: 'var'),
          stmt(printToString(stdCall('post_increment', msg([field('value', ref('i'))])))),
          stmt(printToString(ref('i'))),
        ]),
      ]);
    ...
    test('pre_decrement returns new value and mutates', () {
      final program = buildProgram(functions: [
        mainFn([
          ...
          stmt(printToString(stdCall('pre_decrement', msg([field('value', ref('i'))])))),
          stmt(printToString(ref('i'))),
        ]),
      ]);
    ...
    test('post_decrement returns old value but mutates', () {
      final program = buildProgram(functions: [
        mainFn([
          letStmt('i', literal(3), keyword: 'var'),
          stmt(printToString(stdCall('post_decrement', msg([field('value', ref('i'))])))),
          stmt(printToString(ref('i'))),
        ]),
      ]);
  ...
  group('engine: null safety', () {
    ...
    test('null_coalesce returns right when left is null', () {
      final p = buildProgram(stdFunctions: nullFns, functions: [
        mainFn([
          letStmt('x', litNull()),
          stmt(printToString(stdCall('null_coalesce', msg([field('left', ref('x')), field('right', literal(42))])))),
        ]),
      ]);
    ...
    test('null_coalesce returns left when not null', () {
      final p = buildProgram(stdFunctions: nullFns, functions: [
        mainFn([
          letStmt('x', literal(7)),
          stmt(printToString(stdCall('null_coalesce', msg([field('left', ref('x')), field('right', literal(99))])))),
        ]),
      ]);
      expect(runAndCapture(p), ['7']);
    });
    ...
    test('null_check passes through non-null', () {
      final p = buildProgram(stdFunctions: nullFns, functions: [
        mainFn([
          stmt(printToString(stdCall('null_check', msg([field('value', literal(5))])))),
        ]),
      ]);
      expect(runAndCapture(p), ['5']);
    });
    ...
    test('null_aware_access returns null on null target', () {
      final p = buildProgram(stdFunctions: nullFns, functions: [
        mainFn([
          letStmt('obj', litNull()),
          stmt(printToString(stdCall('null_coalesce', msg([
            field('left', stdCall('null_aware_access', msg([field('target', ref('obj')), field('field', literal('name'))]))),
  ...
  group('engine: type checks', () {
    ...
    test('is int — true', () {
      final p = buildProgram(stdFunctions: typeFns, functions: [
        mainFn([stmt(printToString(stdCall('is', msg([field('value', literal(42)), field('type', literal('int'))]))))]),
      ]);
      expect(runAndCapture(p), ['true']);
    });
    ...
    test('is String — false for int', () {
      final p = buildProgram(stdFunctions: typeFns, functions: [
        mainFn([stmt(printToString(stdCall('is', msg([field('value', literal(42)), field('type', literal('String'))]))))]),
      ]);
      expect(runAndCapture(p), ['false']);
    });
    ...
    test('is_not int — false for int', () {
      final p = buildProgram(stdFunctions: typeFns, functions: [
        mainFn([stmt(printToString(stdCall('is_not', msg([field('value', literal(42)), field('type', literal('int'))]))))]),
      ]);
      expect(runAndCapture(p), ['false']);
    });
    ...
    test('is bool — true', () {
      final p = buildProgram(stdFunctions: typeFns, functions: [
        mainFn([stmt(printToString(stdCall('is', msg([field('value', literal(true)), field('type', literal('bool'))]))))]),
      ]);
      expect(runAndCapture(p), ['true']);
    });
    ...
    test('is List — true', () {
      final p = buildProgram(stdFunctions: typeFns, functions: [
        mainFn([stmt(printToString(stdCall('is', msg([field('value', listLit([literal(1)])), field('type', literal('List'))]))))]),
      ]);
      expect(runAndCapture(p), ['true']);
    });
    ...
    test('as is a no-op passthrough', () {
      final p = buildProgram(stdFunctions: typeFns, functions: [
        mainFn([stmt(printToString(stdCall('as', msg([field('value', literal(42))]))))]),
      ]);
      expect(runAndCapture(p), ['42']);
    });
  });
  ...
  group('engine: map and set creation', () {
    ...
    test('map_create builds a map accessible by key', () {
      final p = buildProgram(stdFunctions: collFns, functions: [
        mainFn([
          letStmt('m', stdCall('map_create', msg([
            field('entries', listLit([
              msg([field('name', literal('a')), field('value', literal(1))]),
              msg([field('name', literal('b')), field('value', literal(2))]),
            ])),
          ]))),
    ...
    test('set_create from list', () {
      final p = buildProgram(stdFunctions: collFns, functions: [
        mainFn([
          letStmt('s', stdCall('set_create', msg([
            field('elements', listLit([literal(1), literal(2), literal(2), literal(3)])),
          ]))),
  ...
  group('engine: short-circuit and / or', () {
    test('and: false && _ = false (does not eval right)', () {
      ...
      final program = buildProgram(functions: [
        mainFn([
          stmt(printToString(stdCall('and', msg([
    ...
    test('or: true || _ = true', () {
      final program = buildProgram(functions: [
        mainFn([
          stmt(printToString(stdCall('or', msg([
            field('left', literal(true)),
  ...
  group('engine: profiling', () {
    test('profiling disabled by default — report is empty', () {
      final program = buildProgram(functions: [mainFn([stmt(printStr('x'))])]);
    ...
    test('profiling enabled — counts std function calls', () {
      final program = buildProgram(functions: [
        mainFn([
          stmt(printStr('a')),
          stmt(printStr('b')),
          stmt(printToString(stdCall('add', msg([field('left', literal(1)), field('right', literal(2))])))),
        ]),
      ]);
    ...
    test('profiling counts string ops correctly', () {
      ...
      final program = buildProgram(stdFunctions: strFns, functions: [
        mainFn([
          stmt(printExpr(stdCall('string_to_upper', msg([field('value', literal('hi'))])))),
          stmt(printExpr(stdCall('string_to_upper', msg([field('value', literal('bye'))])))),
          stmt(printExpr(stdCall('string_to_lower', msg([field('value', literal('X'))])))),
        ]),
      ]);
      final engine = BallEngine(program, stdout: (_) {}, enableProfiling: true);
      engine.run();
  ...
  group('engine: call cache', () {
    test('repeated calls to user function produce correct results', () {
      final program = buildProgram(functions: [
        functionDef('square',
          ...
          outputType: 'int',
          params: [{'name': 'n', 'type': 'int'}],
          body: stdCall('multiply', msg([field('left', ref('n')), field('right', ref('n'))])),
        ),
        mainFn([
    ...
    test('call cache handles multiple distinct functions', () {
      final program = buildProgram(functions: [
        functionDef('double_it',
          inputType: 'int',
          params: [{'name': 'n', 'type': 'int'}],
          body: stdCall('multiply', msg([field('left', ref('n')), field('right', literal(2))])),
        ),
        functionDef('triple_it',
          inputType: 'int',
          params: [{'name': 'n', 'type': 'int'}],
          body: stdCall('multiply', msg([field('left', ref('n')), field('right', literal(3))])),
        ),
        mainFn([
        ...
        mainFn([
          letStmt('r', {
            'block': {
              ...
              'result': stdCall('add', msg([field('left', ref('a')), field('right', ref('b'))])),
            },
          }),
  ...
  group('engine: list literals', () {
    test('creates and accesses list', () {
      final indexFns = [{'name': 'index', 'isBase': true}];
      final program = buildProgram(stdFunctions: indexFns, functions: [
        mainFn([
        ...
        'modules': [
          ...
          },
          {
            ...
            'functions': [
              ...
              functionDef(
                ...
                body: stdCall('math_sqrt', msg([
                  field('value', stdCall('add', msg([
                    field('left',  call('square', module: 'geometry', input: ref('a'))),
                    field('right', call('square', module: 'geometry', input: ref('b'))),
                  ]))),
                ])),
              ),
            ],
          },
          {
            ...
            'functions': [
              mainFn([
                // hypotenuse(3, 4) = 5.0
                stmt(printToString(call('hypotenuse', module: 'geometry',
                  input: msg([field('arg0', literal(3.0)), field('arg1', literal(4.0))])))),
                // square(9) = 81.0
                stmt(printToString(call('square', module: 'geometry', input: literal(9.0)))),
              ]),
            ],
          },
        ],
          ...
          {'name': 'math_utils', 'functions': [
            ...
            functionDef('cube',
              ...
              body: stdCall('multiply', msg([
                field('left', call('square', module: 'math_utils', input: ref('n'))),
                field('right', ref('n')),
              ])),
            ),
          ]},
          {'name': 'main', 'functions': [
            mainFn([stmt(printToString(call('cube', module: 'math_utils', input: literal(3))))]),
          ]},
        ],
    ...
    test('custom handler delegates to std via BallCallable', () {
      ...
      final json = {
        ...
        'modules': [
          ...
          {'name': 'mymath', 'functions': [
            {'name': 'abs_diff', 'isBase': true},
          ]},
          {'name': 'main', 'functions': [
            mainFn([
              stmt(printToString(call('abs_diff', module: 'mymath',
                input: msg([field('a', literal(3)), field('b', literal(10))])))),
              stmt(printToString(call('abs_diff', module: 'mymath',
                input: msg([field('a', literal(10)), field('b', literal(3))])))),
            ]),
          ]},
        ],
        'entryModule': 'main', 'entryFunction': 'main',
    ...
    test('BallCallable can call user-defined functions too', () {
      ...
      final json = {
        ...
        'modules': [
          ...
          {'name': 'ops', 'functions': [
            {'name': 'quadruple', 'isBase': true}, // delegates to main.double_it twice
          ]},
          {'name': 'main', 'functions': [
            functionDef('double_it',
              inputType: 'int',
            ...
            mainFn([
              stmt(printToString(call('quadruple', module: 'ops', input: literal(5)))),
            ]),
          ]},
        ],
        'entryModule': 'main', 'entryFunction': 'main',
      ...
      final prog = Program()..mergeFromProto3Json(json);
      final lines = runAndCapture(prog, handlers: [StdModuleHandler(), delegateHandler]);
  ...
  group('engine: StdModuleHandler.registerComposer', () {
    test('registerComposer can call back into std', () {
      ...
      final json = {
        ...
        'modules': [
          ...
          {'name': 'main', 'functions': [
            mainFn([
              stmt(printToString(stdCall('sum_of_squares',
    ...
    test('registerComposer overrides a built-in', () {
      ...
      final program = buildProgram(functions: [
        mainFn([stmt(printToString(
          stdCall('add', msg([field('left', literal(1)), field('right', literal(2))])),
        ))]),
      ]);
      final lines = <String>[];
    ...
    test('registerComposer and register coexist', () {
      final std = StdModuleHandler()
        ...
        ..registerComposer('my_doubled', (input, engine) =>
            (engine('std', 'my_const', null) as int) * 2);
      final json = {
        'name': 'test', 'version': '1.0.0',
        'modules': [
          {'name': 'std', 'functions': [
  ...
  group('engine: callFunction() public bridge', () {
    ...
    test('callFunction invokes a std base function', () {
      ...
      final engine = BallEngine(program, stdout: (_) {});
      engine.run();
      final result = engine.callFunction('std', 'add', {'left': 10, 'right': 32});
      expect(result, 42);
    });
  ...
  group('engine: BallModuleHandler extensibility', () {
            ...
            'functions': [
              {
                ...
                'body': {
                  'block': {
                    'statements': [
                      stmt(printToString(call('double_val', module: 'math2', input: literal(7)))),
                      stmt(printToString(call('square',     module: 'math2', input: literal(5)))),
                    ],
                  },
                },
              },
    ...
    test('custom handler is queried before unknown-module error', () {
      ...
      final json = {
        ...
        'version': '1.0.0',
        'modules': [
          {'name': 'math2', 'functions': [{'name': 'double_val', 'isBase': true}]},
          {'name': 'std',   'functions': [{'name': 'print', 'isBase': true}, {'name': 'to_string', 'isBase': true}]},
          {'name': 'main',  'functions': [
            mainFn([stmt(printToString(call('double_val', module: 'math2', input: literal(3))))]),
          ]},
        ],
        'entryModule': 'main',
        'entryFunction': 'main',
      ...
      final prog = Program()..mergeFromProto3Json(json);
      final lines = <String>[];
      BallEngine(prog, stdout: lines.add, moduleHandlers: [StdModuleHandler(), math2]).run();
      expect(lines, ['6']);
    });
    ...
    test('throws BallRuntimeError when no handler matches the module', () {
      final json = {
        'name': 'test', 'version': '1.0.0',
        'modules': [
          {'name': 'ghost', 'functions': [{'name': 'foo', 'isBase': true}]},
          {'name': 'main',  'functions': [
  ...
  group('engine: StdModuleHandler customisation', () {
    test('register adds a new std function', () {
      ...
      final json = {
        ...
        'modules': [
          ...
          {'name': 'main', 'functions': [
            mainFn([stmt(printExpr(stdCall('double_str', msg([field('value', literal('hi'))]))))])
          ]},
        ],
        'entryModule': 'main', 'entryFunction': 'main',
      };
    ...
    test('register overrides an existing function', () {
      ...
      final lines = <String>[];
      BallEngine(program, stdout: lines.add, moduleHandlers: [stdHandler]).run();
      expect(lines, ['999']);
    });
    ...
    test('unregister removes a function — subsequent call throws', () {
      ...
      final program = buildProgram(functions: [
        mainFn([stmt(stdCall('negate', msg([field('value', literal(5))])))]),
      ]);
      expect(
    ...
    test('StdModuleHandler.subset allows included functions', () {
      ...
      final program = buildProgram(functions: [
        mainFn([stmt(printToString(
          stdCall('add', msg([field('left', literal(3)), field('right', literal(4))])),
        ))]),
      ]);
      final lines = <String>[];
      BallEngine(program, stdout: lines.add, moduleHandlers: [stdHandler]).run();
    ...
    test('spy handler counts calls while forwarding to delegate', () {
      ...
      expect(calls, greaterThanOrEqualTo(3)); // at minimum 3 print calls counted
    });
    ...
    test('dart_std module also handled by StdModuleHandler', () {
      final json = {
        'name': 'test', 'version': '1.0.0',
        'modules': [
          {'name': 'dart_std', 'functions': [
            {'name': 'print', 'isBase': true},
            {'name': 'to_string', 'isBase': true},
          ]},
          {'name': 'main', 'functions': [
            mainFn([stmt(dartStdCall('print', msg([field('message', literal('dart_std works!'))])))])
          ]},
        ],
        'entryModule': 'main', 'entryFunction': 'main',
      };
  ...
  group('comprehensive.ball.json', () {
    ...
    setUpAll(() {
      program = loadProgram('../../examples/comprehensive/comprehensive.ball.json');
    });
  ...
  group('engine: std_collections module', () {
    ...
    test('list_push and list_length', () {
      final p = buildProgram(stdFunctions: collectionFns, functions: [
        mainFn([
          letStmt(
              'items',
              stdCall('list_push', msg([
                field('list', listLit([literal(1), literal(2)])),
                field('value', literal(3)),
              ]))),
          stmt(printToString(stdCall('list_length', msg([
    ...
    test('list_pop returns last element', () {
      final p = buildProgram(stdFunctions: collectionFns, functions: [
        mainFn([
          stmt(printToString(stdCall('list_pop', msg([
            field('list', listLit([literal(4), literal(5), literal(6)])),
          ])))),
        ]),
      ]);
      expect(runAndCapture(p), ['6']);
    ...
    test('list_get and list_set', () {
      final p = buildProgram(stdFunctions: collectionFns, functions: [
        mainFn([
          letStmt(
              'updated',
              stdCall('list_set', msg([
                field('list', listLit([literal(10), literal(20), literal(30)])),
                field('index', literal(1)),
                field('value', literal(99)),
              ]))),
          stmt(printToString(stdCall('list_get', msg([
    ...
    test('list_map, list_filter, and list_reduce', () {
      final p = buildProgram(stdFunctions: collectionFns, functions: [
        mainFn([
          letStmt(
              'mapped',
              stdCall('list_map', msg([
                field('list', listLit([literal(1), literal(2), literal(3)])),
                field(
                    'callback',
                    lambdaExpr(stdCall('multiply', msg([
          ...
          letStmt(
              'filtered',
              stdCall('list_filter', msg([
                field('list', listLit([literal(1), literal(2), literal(3), literal(4)])),
                field(
                    'callback',
                    lambdaExpr(stdCall('equals', msg([
                      field(
                          'left',
                          stdCall('modulo', msg([
                            field('left', ref('input')),
          ...
          letStmt(
              'sum',
              stdCall('list_reduce', msg([
                field('list', listLit([literal(1), literal(2), literal(3), literal(4)])),
                field('initial', literal(0)),
                field(
                    'callback',
                    lambdaExpr(stdCall('add', msg([
    ...
    test('list_any, list_all, and list_none', () {
      final predicate = lambdaExpr(stdCall('greater_than', msg([
        field('left', ref('input')),
        field('right', literal(0)),
      ])));
      ...
      final p = buildProgram(stdFunctions: collectionFns, functions: [
        mainFn([
          stmt(printToString(stdCall('list_any', msg([
            field('list', listLit([literal(-1), literal(0), literal(2)])),
            field('callback', predicate),
          ])))),
          stmt(printToString(stdCall('list_all', msg([
            field('list', listLit([literal(1), literal(2), literal(3)])),
            field('callback', predicate),
          ])))),
          stmt(printToString(stdCall('list_none', msg([
            field('list', listLit([literal(-3), literal(-2), literal(-1)])),
            field('callback', predicate),
          ])))),
        ]),
      ]);
    ...
    test('map_get, map_set, map_delete, and map_length', () {
      final p = buildProgram(stdFunctions: collectionFns, functions: [
        mainFn([
          ...
          letStmt(
              'm3',
              stdCall('map_delete', msg([
                field('map', ref('m2')),
                field('key', literal('a')),
              ]))),
          stmt(printToString(stdCall('map_get', msg([
            field('map', ref('m2')),
            field('key', literal('c')),
          ])))),
          stmt(printToString(stdCall('map_contains_key', msg([
            field('map', ref('m2')),
            field('key', literal('c')),
          ])))),
          stmt(printToString(stdCall('map_length', msg([
            field('map', ref('m3')),
          ])))),
        ]),
      ]);
    ...
    test('map_keys, map_values, and string_join', () {
      final p = buildProgram(stdFunctions: collectionFns, functions: [
        mainFn([
          ...
          letStmt('values', stdCall('map_values', msg([field('map', ref('m'))]))),
          stmt(printToString(fieldAcc(ref('keys'), 'length'))),
          stmt(printToString(fieldAcc(ref('values'), 'length'))),
          stmt(printExpr(stdCall('string_join', msg([
            field('list', ref('keys')),
    ...
    test('set_create, set_add, set_union, and set_length', () {
      final p = buildProgram(stdFunctions: collectionFns, functions: [
        mainFn([
          ...
          letStmt(
              'unioned',
              stdCall('set_union', msg([
                field('left', ref('left2')),
                field('right', ref('right')),
              ]))),
          stmt(printToString(stdCall('set_contains', msg([
            field('set', ref('unioned')),
            field('value', literal(4)),
          ])))),
          stmt(printToString(stdCall('set_length', msg([
            field('set', ref('unioned')),
          ])))),
        ]),
      ]);
  ...
  group('engine: std_io module', () {
    test('print_error writes to stderr', () {
      final program = buildProgram(
        functions: [
          mainFn([
            stmt(call('print_error', module: 'std',
                input: msg([field('message', literal('error msg'))]))),
          ]),
        ],
        stdFunctions: [{'name': 'print_error', 'isBase': true}],
      );
    ...
    test('timestamp_ms returns epoch ms', () {
      final program = buildProgram(
        ...
        stdFunctions: [{'name': 'timestamp_ms', 'isBase': true}],
      );
      final lines = runAndCapture(program);
    ...
    test('random_int returns value in range', () {
      final program = buildProgram(
        ...
        stdFunctions: [{'name': 'random_int', 'isBase': true}],
      );
      final lines = runAndCapture(program);
    ...
    test('random_double returns value in [0, 1)', () {
      final program = buildProgram(
        ...
        stdFunctions: [{'name': 'random_double', 'isBase': true}],
      );
      final lines = runAndCapture(program);
    ...
    test('env_get reads environment variable', () {
      final program = buildProgram(
        functions: [
          mainFn([
            stmt(printExpr(call('env_get',
                module: 'std',
                input: msg([field('name', literal('PATH'))])))),
          ]),
        ],
        stdFunctions: [{'name': 'env_get', 'isBase': true}],
      );
      final lines = <String>[];
      final engine = BallEngine(program,
          stdout: lines.add,
    ...
    test('args_get returns provided args', () {
      final program = buildProgram(
        functions: [
          mainFn([
            stmt(printToString(call('args_get', module: 'std'))),
          ]),
        ],
        stdFunctions: [{'name': 'args_get', 'isBase': true}],
      );
      final lines = <String>[];
      final engine = BallEngine(program,
          stdout: lines.add,
  ...
  group('engine: std_convert module', () {
      ...
      final program = buildProgram(
        functions: [
          mainFn([
            letStmt(
                'm',
                msg([field('x', literal(42)), field('y', literal('hello'))],
                    typeName: 'TestObj')),
            letStmt(
                'j',
                call('json_encode',
                    module: 'std', input: msg([field('value', ref('m'))]))),
            stmt(printExpr(ref('j'))),
          ]),
        ],
        stdFunctions: [{'name': 'json_encode', 'isBase': true}],
      );
      final lines = runAndCapture(program);
    ...
    test('json_decode parses a string', () {
      final program = buildProgram(
        ...
        stdFunctions: [{'name': 'json_decode', 'isBase': true}],
      );
      final lines = runAndCapture(program);
  ...
  group('engine: std_time module', () {
    test('now returns epoch ms', () {
      final program = buildProgram(
        ...
        stdFunctions: [{'name': 'now', 'isBase': true}],
      );
      final lines = runAndCapture(program);
      final ts = int.parse(lines.first);
    ...
    test('year returns valid year', () {
      final program = buildProgram(
        ...
        stdFunctions: [{'name': 'year', 'isBase': true}],
      );
      final lines = runAndCapture(program);
  ...
  group('engine: pattern matching', () {
    test('type pattern matches int', () {
      final program = buildProgram(
        ...
        stdFunctions: [{'name': 'switch_expr', 'isBase': true}],
      );
      final lines = runAndCapture(program);
  ...
  group('engine: std_fs module', () {
    ...
    test('file_exists returns true for existing file', () {
      ...
      final program = buildProgram(
        ...
        stdFunctions: [{'name': 'file_exists', 'isBase': true}],
      );
      final lines = runAndCapture(program);
