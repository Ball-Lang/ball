...
# Regenerate the Go self-hosted CLI core (dart/self_host/cli.ball.pb ->
# go/cli/compiled/compiled_cli.go, gitignored, only in the build under the
# `clicore` build tag — deliberately independent of `selfhost`, since
# info/validate/tree/version never touch the interpreter. See go/cli/AGENTS.md.
cd dart && dart run compiler/tool/gen_cli_json.dart
cd ../go/cli && go run ./cmd/regen
go build -tags clicore ./... && go test -tags clicore ./...   # the golden-parity gate
...
python -m conformance.runner                           # Results: 344 passed, 0 failed, 344 total
...
- `ball-lang-engine` — self-hosted engine (SKILL.md Phase 4 Option B), same approach as TS/C++:
  ...
  with Dart-identical output (`Results: 344 passed, 0 failed, 344 total`; the 4 golden-less
...
- `Ball.Engine` (`engine/`) — self-hosted engine (SKILL.md Phase 4 Option B), same approach as
  ...
  Dart-identical output (`Results: 344 passed, 0 failed, 344 total`; the 4 golden-less
...
- `go/engine` — self-hosted engine (SKILL.md Phase 4 Option B), same approach as TS/C++/Rust/C#:
  ...
  conformance corpus with Dart-identical output (`Results: 344 passed, 0 failed, 344 total`; the 4
...
- `go/cli` (module `.../go/cli`, `cmd/ball`) — the `ball` CLI (#437): `run`/`compile`/`encode`/
  ...
  silent success. The self-hosted cli-core verbs `info`/`validate`/`tree`/`version` (#570) sit behind
  their OWN `clicore` build tag, deliberately independent of `selfhost` (they never touch the
  interpreter — the Go analog of Rust's `cli_core` feature and C#'s `-p:CliCore=true`); they are
  dispatched and listed in `--help` in every build and fail loud with a regenerate hint without the
  generated `go/cli/compiled/compiled_cli.go`. See `go/cli/AGENTS.md`.
...
- `python/engine` — self-hosted engine (SKILL.md Phase 4 Option B), same approach as
  ...
  compiled engine runs the whole conformance corpus with Dart-identical output (`Results: 344
  passed, 0 failed, 344 total`; the 4 golden-less resource-limit/sandbox fixtures are documented
