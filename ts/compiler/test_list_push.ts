...
// In-place list append: mutates target by pushing all elements of items.
// Used for Dart's List.addAll semantics where callers sharing the same list
// reference must observe the appended elements (unlike __ball_concat which
// creates a new array and breaks reference identity).
...
function main(): any {
  let buffer = [1, 2];
  __ball_push_all(buffer, [3, 4]);
  console.log(__ball_to_string(__ball_to_string(buffer.length)));
}
main();
