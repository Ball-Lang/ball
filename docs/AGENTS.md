...
| `SINK_DESIGN.md` | Design record for mutable output sinks (`std.sink_create`/`sink_write`/`sink_to_string`, issue #630): the normative runtime contract (tagged, **reference-semantic**) with each target's required backing, how every source language's sink construct maps on, and the Rust encoder's syntax-only `write!`/`writeln!` rule. Read before adding sink support to another encoder, compiler or target |
...
| `articles/introducing-ball.md` | Public "Introducing Ball" article (linked from the README). Editorial revisions tracked in [#136](https://github.com/Ball-Lang/ball/issues/136) (was `ARTICLE_REVIEW.md`) — applied in full. |
