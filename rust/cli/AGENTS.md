...
`ball-engine`/`ball-compiler`/`ball-encoder` (issue #41), plus the self-hosted cli-core verbs
...
- `src/loader.rs` — `load_engine(path)`: format-sniffs `.bin` (binary protobuf) vs. anything
  ...
  by every subcommand, per the issue's "the loader in ball-engine handles both" note. `info`/
...
exactly) compiles it via `ball-compiler` in **library mode** into `src/compiled_cli.rs`: since
...
BallValue) -> BallValue`, etc. — no runtime-driving wrapper like `ball-engine`'s
...
- Gated behind `ball-cli`'s own `cli_core` Cargo feature (`Cargo.toml`), off by default for the
...
- **`auditReport` is intentionally excluded** from `compiled_cli.rs` (issue #365, `#362`
  ...
  every language) nor `ball-compiler` (general-purpose). No `ball audit` subcommand exists in
  `ball-cli`.
...
cargo build -p ball-cli --features cli_core     # after the two regen commands above
cargo test -p ball-cli                          # unit tests + tests/*.rs integration tests
cargo test -p ball-cli --features cli_core      # additionally exercises the golden parity gate
cargo fmt -p ball-cli --check
cargo clippy -p ball-cli --all-targets -- -D warnings
cargo clippy -p ball-cli --all-targets --features cli_core -- -D warnings
```
...
- `check` does not attempt to run the program — only `ball-compiler`-shaped structural
  validation, plus an opt-in `--compile` dry-run. It never drives `ball-engine`. (It predates
