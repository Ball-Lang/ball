...
public class RealWorldSweepTests(ITestOutputHelper output)
{
    ...
    [
        ...
        ("j: 0-argument LINQ terminals (.Last(), .Any())", "j_zero_arg_linq_terminals.cs", EncodeMode.Program, true,
            "CLOSED by #492 slice 3b — `DispatchInstanceOrBuiltinMethod`'s arity windows carried `First`/`FirstOrDefault` at BOTH arities but `Any` only at 1 and `Last` at none, so the 0-arg spellings fell through to the same generic `unsupported method call` throw: `.Last()` now routes to the declared `std_collections.list_last` (same throw-on-empty contract as C#'s own `.Last()`) and 0-arg `.Any()` to `std.not(std_collections.list_is_empty(...))`. Every `*OrDefault` spelling stays a LOUD error on purpose — `FirstOrDefault` was un-routed from both of `First`'s arms by #588 — see ZeroArgLinqTerminalTests and FirstOrDefaultContractTests"),
        ("k: string.Join(separator, values)", "k_string_join.cs", EncodeMode.Program, true,
