...
`csharp/engine/src/CompiledEngine.cs` is generated only under `-p:SelfHost=true`. In the **default** build, `BallEngine.Run()` unconditionally throws `SelfHostPendingException` (`BallEngine.cs:80-87`), because the compiled engine it drives is a gitignored artifact that only that flag generates. Built WITH the flag, the self-hosted engine runs at Dart parity (`csharp/AGENTS.md`, `Results: 362 passed, 0 failed, 362 total`), but `Run()` still takes **zero parameters**:
