// Ball Language Schema
// A programming language represented entirely as protobuf messages.
// Ball enables code-as-data, dynamic code generation, and cross-language compilation.
//
// Module: buf.build/ball-lang/ball
// Managed by Buf — see buf.yaml for module config and buf.gen.yaml for codegen.

// @generated by protoc-gen-es v2.12.0
// @generated from file ball/v1/ball.proto (package ball.v1, syntax proto3)
/* eslint-disable */

import type { GenEnum, GenFile, GenMessage } from "@bufbuild/protobuf/codegenv2";
import type { JsonObject, Message } from "@bufbuild/protobuf";
import type { DescriptorProto, EnumDescriptorProto } from "@bufbuild/protobuf/wkt";

/**
 * Describes the file ball/v1/ball.proto.
 */
export declare const file_ball_v1_ball: GenFile;

/**
 * A complete ball program containing all modules, types, and functions.
 *
 * @generated from message ball.v1.Program
 */
export declare type Program = Message<"ball.v1.Program"> & {
  /**
   * Human-readable program name
   *
   * @generated from field: string name = 1;
   */
  name: string;

  /**
   * Semantic version string (e.g., "1.0.0")
   *
   * @generated from field: string version = 2;
   */
  version: string;

  /**
   * All modules in this program (including base modules like "std")
   *
   * @generated from field: repeated ball.v1.Module modules = 3;
   */
  modules: Module[];

  /**
   * The module containing the entry point function
   *
   * @generated from field: string entry_module = 4;
   */
  entryModule: string;

  /**
   * The function to execute as the program's entry point
   *
   * @generated from field: string entry_function = 5;
   */
  entryFunction: string;

  /**
   * Arbitrary metadata (author, license, etc.) — supports infinite nesting.
   *
   * @generated from field: google.protobuf.Struct metadata = 6;
   */
  metadata?: JsonObject | undefined;
};

/**
 * Describes the message ball.v1.Program.
 * Use `create(ProgramSchema)` to create a new message.
 */
export declare const ProgramSchema: GenMessage<Program>;

/**
 * A module groups related type definitions and functions.
 * Modules can import other modules to use their types and functions.
 * Base modules (like "std") provide platform-specific implementations.
 *
 * @generated from message ball.v1.Module
 */
export declare type Module = Message<"ball.v1.Module"> & {
  /**
   * Unique module name (e.g., "std", "main", "my_lib")
   *
   * @generated from field: string name = 1;
   */
  name: string;

  /**
   * Enum types defined using protobuf's own enum descriptor format.
   *
   * @generated from field: repeated google.protobuf.EnumDescriptorProto enums = 7;
   */
  enums: EnumDescriptorProto[];

  /**
   * Function definitions in this module
   *
   * @generated from field: repeated ball.v1.FunctionDefinition functions = 3;
   */
  functions: FunctionDefinition[];

  /**
   * Human-readable description
   *
   * @generated from field: string description = 5;
   */
  description: string;

  /**
   * Arbitrary metadata — supports infinite nesting.
   *
   * @generated from field: google.protobuf.Struct metadata = 6;
   */
  metadata?: JsonObject | undefined;

  /**
   * Structured module imports with source resolution and integrity verification.
   *
   * Each ModuleImport specifies where to find the module (HTTP URL,
   * local file, inline bytes/JSON, or git repo) and an optional
   * content hash for integrity verification.
   *
   * @generated from field: repeated ball.v1.ModuleImport module_imports = 4;
   */
  moduleImports: ModuleImport[];

  /**
   * First-class type definitions (replaces the _meta_ function hack).
   * Each TypeDefinition has a name, protobuf descriptor for fields,
   * generic type parameters, and a metadata bag for cosmetic hints
   * (kind, superclass, interfaces, mixins, visibility, etc.).
   *
   * @generated from field: repeated ball.v1.TypeDefinition type_defs = 8;
   */
  typeDefs: TypeDefinition[];

  /**
   * Type aliases (e.g., C++ `using`, Rust `type`, TypeScript `type`).
   *
   * @generated from field: repeated ball.v1.TypeAlias type_aliases = 11;
   */
  typeAliases: TypeAlias[];

  /**
   * Module-level constants (e.g., `const pi = 3.14159`).
   *
   * @generated from field: repeated ball.v1.Constant module_constants = 12;
   */
  moduleConstants: Constant[];

  /**
   * Asset files embedded in this module (images, JSON fixtures, etc.).
   * Any file can be stored as an asset; the path is relative to the
   * package root.
   *
   * @generated from field: repeated ball.v1.ModuleAsset assets = 13;
   */
  assets: ModuleAsset[];
};

/**
 * Describes the message ball.v1.Module.
 * Use `create(ModuleSchema)` to create a new message.
 */
export declare const ModuleSchema: GenMessage<Module>;

/**
 * An asset file embedded in a ball module.
 *
 * Assets can be any file (images, JSON fixtures, templates, etc.) and are
 * stored as raw bytes alongside the module's code.  The path is relative to
 * the package root (e.g. "test/fixtures/users.json", "assets/logo.png").
 *
 * @generated from message ball.v1.ModuleAsset
 */
export declare type ModuleAsset = Message<"ball.v1.ModuleAsset"> & {
  /**
   * Relative path of the asset within the package (forward-slash separated).
   * Example: "test/fixtures/users.json", "web/icons/favicon.ico"
   *
   * @generated from field: string path = 1;
   */
  path: string;

  /**
   * Raw file content.
   *
   * @generated from field: bytes content = 2;
   */
  content: Uint8Array;

  /**
   * MIME type hint (e.g. "application/json", "image/png", "text/plain").
   * Empty = infer from file extension.
   *
   * @generated from field: string media_type = 3;
   */
  mediaType: string;

  /**
   * Arbitrary metadata (e.g. encoding hints, generation source).
   *
   * @generated from field: google.protobuf.Struct metadata = 4;
   */
  metadata?: JsonObject | undefined;
};

/**
 * Describes the message ball.v1.ModuleAsset.
 * Use `create(ModuleAssetSchema)` to create a new message.
 */
export declare const ModuleAssetSchema: GenMessage<ModuleAsset>;

/**
 * Specifies how to resolve and load a module dependency.
 *
 * @generated from message ball.v1.ModuleImport
 */
export declare type ModuleImport = Message<"ball.v1.ModuleImport"> & {
  /**
   * Local alias used to reference this module in FunctionCall.module.
   * Freely chosen by the importing module — does not need to match
   * the imported module's own Module.name.
   *
   * @generated from field: string name = 1;
   */
  name: string;

  /**
   * Content integrity hash for verification.
   *
   * Format: "<algorithm>:<hex-encoded-hash>"
   * Example: "sha256:e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"
   *
   * When present, the resolver serializes the resolved Module to
   * canonical protobuf binary format and compares its SHA-256 hash.
   * A mismatch causes resolution failure, protecting against
   * supply-chain attacks and accidental corruption.
   *
   * When absent, no integrity check is performed.
   *
   * @generated from field: string integrity = 2;
   */
  integrity: string;

  /**
   * Arbitrary metadata (e.g., reason for dependency, override notes).
   *
   * @generated from field: google.protobuf.Struct metadata = 3;
   */
  metadata?: JsonObject | undefined;

  /**
   * Source from which to resolve the module.
   *
   * @generated from oneof ball.v1.ModuleImport.source
   */
  source: {
    /**
     * Download from an HTTP/HTTPS URL.
     *
     * @generated from field: ball.v1.HttpSource http = 4;
     */
    value: HttpSource;
    case: "http";
  } | {
    /**
     * Load from a local filesystem path.
     *
     * @generated from field: ball.v1.FileSource file = 5;
     */
    value: FileSource;
    case: "file";
  } | {
    /**
     * Embedded directly in the import (raw protobuf or JSON).
     *
     * @generated from field: ball.v1.InlineSource inline = 6;
     */
    value: InlineSource;
    case: "inline";
  } | {
    /**
     * Resolve from a git repository.
     *
     * @generated from field: ball.v1.GitSource git = 7;
     */
    value: GitSource;
    case: "git";
  } | {
    /**
     * Resolve from a language-native package registry (pub, npm, nuget, etc.).
     *
     * @generated from field: ball.v1.RegistrySource registry = 8;
     */
    value: RegistrySource;
    case: "registry";
  } | { case: undefined; value?: undefined };
};

/**
 * Describes the message ball.v1.ModuleImport.
 * Use `create(ModuleImportSchema)` to create a new message.
 */
export declare const ModuleImportSchema: GenMessage<ModuleImport>;

/**
 * An HTTP/HTTPS source for direct module download.
 *
 * The URL must point to a valid serialized ball Module in either
 * protobuf binary or JSON format.
 *
 * Example:
 *   url: "https://example.com/modules/my_lib/v1.0.0/module.ball.bin"
 *   encoding: MODULE_ENCODING_PROTO
 *
 * @generated from message ball.v1.HttpSource
 */
export declare type HttpSource = Message<"ball.v1.HttpSource"> & {
  /**
   * Full URL to the module file.
   *
   * @generated from field: string url = 1;
   */
  url: string;

  /**
   * Expected serialization format of the response body.
   * MODULE_ENCODING_UNSPECIFIED = auto-detect from Content-Type or extension.
   *
   * @generated from field: ball.v1.ModuleEncoding encoding = 2;
   */
  encoding: ModuleEncoding;

  /**
   * Optional HTTP headers for authentication or custom routing.
   * Example: {"Authorization": "Bearer ${TOKEN}"}
   *
   * @generated from field: map<string, string> headers = 3;
   */
  headers: { [key: string]: string };
};

/**
 * Describes the message ball.v1.HttpSource.
 * Use `create(HttpSourceSchema)` to create a new message.
 */
export declare const HttpSourceSchema: GenMessage<HttpSource>;

/**
 * A local filesystem source for development dependencies.
 *
 * Paths can be absolute or relative to the importing module's location.
 * This source type is intended for local development — production
 * programs should use registry or HTTP sources.
 *
 * Example:
 *   path: "../my_lib/module.ball.json"
 *   encoding: MODULE_ENCODING_JSON
 *
 * @generated from message ball.v1.FileSource
 */
export declare type FileSource = Message<"ball.v1.FileSource"> & {
  /**
   * Path to the module file.
   * Absolute or relative to the importing program/module location.
   *
   * @generated from field: string path = 1;
   */
  path: string;

  /**
   * Expected serialization format.
   * MODULE_ENCODING_UNSPECIFIED = auto-detect from file extension or first few bytes of the file.
   *
   * @generated from field: ball.v1.ModuleEncoding encoding = 2;
   */
  encoding: ModuleEncoding;
};

/**
 * Describes the message ball.v1.FileSource.
 * Use `create(FileSourceSchema)` to create a new message.
 */
export declare const FileSourceSchema: GenMessage<FileSource>;

/**
 * An inline/embedded module source.
 *
 * The entire module is embedded directly in the import, either as
 * raw protobuf binary bytes or as a JSON string. Useful for:
 *   - Self-contained programs (single-file distribution)
 *   - Code generation pipelines (embed generated modules)
 *   - Testing (inline test fixtures)
 *
 * @generated from message ball.v1.InlineSource
 */
export declare type InlineSource = Message<"ball.v1.InlineSource"> & {
  /**
   * @generated from oneof ball.v1.InlineSource.content
   */
  content: {
    /**
     * Raw protobuf binary-encoded Module.
     *
     * @generated from field: bytes proto_bytes = 1;
     */
    value: Uint8Array;
    case: "protoBytes";
  } | {
    /**
     * JSON-encoded Module string (protobuf JSON mapping).
     *
     * @generated from field: string json = 2;
     */
    value: string;
    case: "json";
  } | { case: undefined; value?: undefined };
};

/**
 * Describes the message ball.v1.InlineSource.
 * Use `create(InlineSourceSchema)` to create a new message.
 */
export declare const InlineSourceSchema: GenMessage<InlineSource>;

/**
 * A git repository source for importing modules from version control.
 *
 * The resolver clones (or shallow-fetches) the repository at the
 * specified ref and reads the module file at the given path.
 *
 * Example:
 *   url: "https://github.com/ball-lang/std-extended.git"
 *   ref: "v1.2.0"
 *   path: "modules/std_extended/module.ball.json"
 *
 * @generated from message ball.v1.GitSource
 */
export declare type GitSource = Message<"ball.v1.GitSource"> & {
  /**
   * Git repository URL (HTTPS or SSH).
   * Example: "https://github.com/ball-lang/std.git"
   *          "git@github.com:ball-lang/std.git"
   *
   * @generated from field: string url = 1;
   */
  url: string;

  /**
   * Git ref to resolve: branch name, tag, or full commit SHA.
   * Tags are preferred for versioned releases.
   * Example: "v1.2.0", "main", "abc123def456..."
   *
   * @generated from field: string ref = 2;
   */
  ref: string;

  /**
   * Path within the repository to the module file.
   * Example: "src/module.ball.json"
   *
   * @generated from field: string path = 3;
   */
  path: string;

  /**
   * Expected serialization format of the file.
   * MODULE_ENCODING_UNSPECIFIED = auto-detect from file extension.
   *
   * @generated from field: ball.v1.ModuleEncoding encoding = 4;
   */
  encoding: ModuleEncoding;
};

/**
 * Describes the message ball.v1.GitSource.
 * Use `create(GitSourceSchema)` to create a new message.
 */
export declare const GitSourceSchema: GenMessage<GitSource>;

/**
 * A language-native package registry source.
 *
 * Ball modules can be published inside native packages on any supported
 * registry. The resolver fetches the package archive, extracts the Ball
 * module file at `module_path`, and inlines it into the program.
 *
 * Example:
 *   registry: REGISTRY_PUB
 *   package: "ball_std_extended"
 *   version: "^1.0.0"
 *   module_path: "lib/module.ball.bin"
 *
 * @generated from message ball.v1.RegistrySource
 */
export declare type RegistrySource = Message<"ball.v1.RegistrySource"> & {
  /**
   * Which registry to resolve from.
   *
   * @generated from field: ball.v1.Registry registry = 1;
   */
  registry: Registry;

  /**
   * Package name as it appears on the registry.
   * Examples: "ball_math_utils" (pub), "@ball/my-module" (npm),
   *           "Ball.MyModule" (nuget), "dev.ball:my-module" (maven).
   *
   * @generated from field: string package = 2;
   */
  package: string;

  /**
   * Semver version constraint.
   * Examples: "1.0.0", "^1.0.0", ">=1.0.0 <2.0.0"
   * Empty = latest stable.
   *
   * @generated from field: string version = 3;
   */
  version: string;

  /**
   * Path to the .ball.bin or .ball.json file inside the package archive.
   * If empty, the resolver uses the registry-specific default path convention
   * (e.g. "lib/module.ball.bin" for pub, "package/module.ball.bin" for npm).
   *
   * @generated from field: string module_path = 4;
   */
  modulePath: string;

  /**
   * Expected serialization format of the module file.
   *
   * @generated from field: ball.v1.ModuleEncoding encoding = 5;
   */
  encoding: ModuleEncoding;

  /**
   * Custom registry URL (overrides the default for the registry type).
   * Use for private or self-hosted registries.
   * Example: "https://pub.my-company.com"
   *
   * @generated from field: string registry_url = 6;
   */
  registryUrl: string;
};

/**
 * Describes the message ball.v1.RegistrySource.
 * Use `create(RegistrySourceSchema)` to create a new message.
 */
export declare const RegistrySourceSchema: GenMessage<RegistrySource>;

/**
 * A structured type reference for use in type checks, function call type
 * arguments, and function signatures. Replaces raw Dart-flavored type strings
 * (e.g. "Box<int>") with a structured, target-agnostic representation that
 * every compiler can interpret without parsing language-specific syntax.
 *
 * Examples:
 *   int            → {name: "int"}
 *   Box<int>       → {name: "Box", type_args: [{name: "int"}]}
 *   Map<String, List<int?>> → {name: "Map", type_args: [
 *     {name: "String"},
 *     {name: "List", type_args: [{name: "int", nullable: true}]}
 *   ]}
 *
 * @generated from message ball.v1.TypeRef
 */
export declare type TypeRef = Message<"ball.v1.TypeRef"> & {
  /**
   * Base type name (e.g., "int", "String", "Box", "List").
   * For module-qualified types: "module:TypeName" (e.g., "main:Container").
   *
   * @generated from field: string name = 1;
   */
  name: string;

  /**
   * Generic type arguments. Empty for non-generic types.
   *
   * @generated from field: repeated ball.v1.TypeRef type_args = 2;
   */
  typeArgs: TypeRef[];

  /**
   * Whether this type is nullable (e.g. int? → nullable = true).
   *
   * @generated from field: bool nullable = 3;
   */
  nullable: boolean;
};

/**
 * Describes the message ball.v1.TypeRef.
 * Use `create(TypeRefSchema)` to create a new message.
 */
export declare const TypeRefSchema: GenMessage<TypeRef>;

/**
 * A type parameter placeholder for generic types (e.g. T, K, V).
 * Bounds, variance, and other constraints are cosmetic hints in metadata.
 *
 * @generated from message ball.v1.TypeParameter
 */
export declare type TypeParameter = Message<"ball.v1.TypeParameter"> & {
  /**
   * Type parameter name (e.g. "T", "K", "V")
   *
   * @generated from field: string name = 1;
   */
  name: string;

  /**
   * Cosmetic hints: bounds ("extends Comparable"), variance, covariance, etc.
   *
   * @generated from field: google.protobuf.Struct metadata = 2;
   */
  metadata?: JsonObject | undefined;
};

/**
 * Describes the message ball.v1.TypeParameter.
 * Use `create(TypeParameterSchema)` to create a new message.
 */
export declare const TypeParameterSchema: GenMessage<TypeParameter>;

/**
 * Defines a named type, mirroring the structure of FunctionDefinition.
 *
 * TypeDefinition replaces the `_meta_Foo` function convention with a
 * proper schema-level construct. All cosmetic hints (kind, superclass,
 * interfaces, mixins, visibility, annotations, etc.) go in metadata.
 *
 * @generated from message ball.v1.TypeDefinition
 */
export declare type TypeDefinition = Message<"ball.v1.TypeDefinition"> & {
  /**
   * Type name (unique within its module)
   *
   * @generated from field: string name = 1;
   */
  name: string;

  /**
   * Field definitions using protobuf's own descriptor format
   *
   * @generated from field: google.protobuf.DescriptorProto descriptor = 2;
   */
  descriptor?: DescriptorProto | undefined;

  /**
   * Generic type parameter names (e.g. T, K, V)
   *
   * @generated from field: repeated ball.v1.TypeParameter type_params = 3;
   */
  typeParams: TypeParameter[];

  /**
   * Human-readable description
   *
   * @generated from field: string description = 4;
   */
  description: string;

  /**
   * All cosmetic hints: kind ("class"|"struct"|"trait"|"interface"|...),
   * superclass, interfaces, mixins, visibility, is_abstract, is_sealed,
   * is_final, annotations, fields metadata, etc.
   *
   * @generated from field: google.protobuf.Struct metadata = 5;
   */
  metadata?: JsonObject | undefined;
};

/**
 * Describes the message ball.v1.TypeDefinition.
 * Use `create(TypeDefinitionSchema)` to create a new message.
 */
export declare const TypeDefinitionSchema: GenMessage<TypeDefinition>;

/**
 * A type alias (e.g., C++ `using`, Rust `type`, TypeScript `type`, Dart `typedef`).
 *
 * @generated from message ball.v1.TypeAlias
 */
export declare type TypeAlias = Message<"ball.v1.TypeAlias"> & {
  /**
   * Alias name
   *
   * @generated from field: string name = 1;
   */
  name: string;

  /**
   * The aliased type name
   *
   * @generated from field: string target_type = 2;
   */
  targetType: string;

  /**
   * Generic type parameters
   *
   * @generated from field: repeated ball.v1.TypeParameter type_params = 3;
   */
  typeParams: TypeParameter[];

  /**
   * Cosmetic hints: visibility, language-specific keywords
   *
   * @generated from field: google.protobuf.Struct metadata = 4;
   */
  metadata?: JsonObject | undefined;
};

/**
 * Describes the message ball.v1.TypeAlias.
 * Use `create(TypeAliasSchema)` to create a new message.
 */
export declare const TypeAliasSchema: GenMessage<TypeAlias>;

/**
 * A module-level constant value.
 *
 * @generated from message ball.v1.Constant
 */
export declare type Constant = Message<"ball.v1.Constant"> & {
  /**
   * Constant name
   *
   * @generated from field: string name = 1;
   */
  name: string;

  /**
   * Type name (empty = infer from value)
   *
   * @generated from field: string type = 2;
   */
  type: string;

  /**
   * Constant value expression
   *
   * @generated from field: ball.v1.Expression value = 3;
   */
  value?: Expression | undefined;

  /**
   * Cosmetic hints: visibility, annotations
   *
   * @generated from field: google.protobuf.Struct metadata = 4;
   */
  metadata?: JsonObject | undefined;
};

/**
 * Describes the message ball.v1.Constant.
 * Use `create(ConstantSchema)` to create a new message.
 */
export declare const ConstantSchema: GenMessage<Constant>;

/**
 * Defines a function with a single input type and single output type,
 * following the gRPC pattern. Base functions have no body — their
 * implementation is provided by each target language's compiler.
 *
 * @generated from message ball.v1.FunctionDefinition
 */
export declare type FunctionDefinition = Message<"ball.v1.FunctionDefinition"> & {
  /**
   * Function name (must be unique within its module)
   *
   * @generated from field: string name = 1;
   */
  name: string;

  /**
   * Input type name (empty string = no input / void)
   *
   * @generated from field: string input_type = 2;
   */
  inputType: string;

  /**
   * Output type name (empty string = no output / void)
   *
   * @generated from field: string output_type = 3;
   */
  outputType: string;

  /**
   * Function body expression (absent for base functions)
   *
   * @generated from field: ball.v1.Expression body = 4;
   */
  body?: Expression | undefined;

  /**
   * Human-readable description
   *
   * @generated from field: string description = 5;
   */
  description: string;

  /**
   * If true, this function's implementation is provided by the
   * target platform compiler, not by a ball expression body.
   *
   * @generated from field: bool is_base = 6;
   */
  isBase: boolean;

  /**
   * Arbitrary metadata — supports infinite nesting.
   *
   * @generated from field: google.protobuf.Struct metadata = 7;
   */
  metadata?: JsonObject | undefined;
};

/**
 * Describes the message ball.v1.FunctionDefinition.
 * Use `create(FunctionDefinitionSchema)` to create a new message.
 */
export declare const FunctionDefinitionSchema: GenMessage<FunctionDefinition>;

/**
 * An expression is the fundamental unit of computation in ball.
 * Every computation is represented as an expression tree.
 *
 * @generated from message ball.v1.Expression
 */
export declare type Expression = Message<"ball.v1.Expression"> & {
  /**
   * @generated from oneof ball.v1.Expression.expr
   */
  expr: {
    /**
     * Call a function with an input expression
     *
     * @generated from field: ball.v1.FunctionCall call = 1;
     */
    value: FunctionCall;
    case: "call";
  } | {
    /**
     * A literal value (int, double, string, bool, list)
     *
     * @generated from field: ball.v1.Literal literal = 2;
     */
    value: Literal;
    case: "literal";
  } | {
    /**
     * A reference to a variable by name
     *
     * @generated from field: ball.v1.Reference reference = 3;
     */
    value: Reference;
    case: "reference";
  } | {
    /**
     * Access a field of a message expression
     *
     * @generated from field: ball.v1.FieldAccess field_access = 4;
     */
    value: FieldAccess;
    case: "fieldAccess";
  } | {
    /**
     * Construct a new message instance
     *
     * @generated from field: ball.v1.MessageCreation message_creation = 5;
     */
    value: MessageCreation;
    case: "messageCreation";
  } | {
    /**
     * A block of statements with a result expression
     *
     * @generated from field: ball.v1.Block block = 6;
     */
    value: Block;
    case: "block";
  } | {
    /**
     * An anonymous function / closure (name is empty).
     * Cross-language: JS arrow functions, Python lambdas, Kotlin lambdas,
     * Swift closures, C# delegates/lambdas, Rust closures, etc.
     * A lambda is just a FunctionDefinition with name = "".
     *
     * @generated from field: ball.v1.FunctionDefinition lambda = 7;
     */
    value: FunctionDefinition;
    case: "lambda";
  } | { case: undefined; value?: undefined };
};

/**
 * Describes the message ball.v1.Expression.
 * Use `create(ExpressionSchema)` to create a new message.
 */
export declare const ExpressionSchema: GenMessage<Expression>;

/**
 * Calls a function with a single input expression.
 * The input expression must evaluate to the function's declared input type.
 *
 * @generated from message ball.v1.FunctionCall
 */
export declare type FunctionCall = Message<"ball.v1.FunctionCall"> & {
  /**
   * Module name (empty string = current module)
   *
   * @generated from field: string module = 1;
   */
  module: string;

  /**
   * Function name to call
   *
   * @generated from field: string function = 2;
   */
  function: string;

  /**
   * Input expression (evaluates to the function's input type)
   *
   * @generated from field: ball.v1.Expression input = 3;
   */
  input?: Expression | undefined;

  /**
   * Generic type arguments for this call (e.g., identity<int>(42)).
   * Replaces the former "__type_args__" string convention.
   *
   * @generated from field: repeated ball.v1.TypeRef type_args = 4;
   */
  typeArgs: TypeRef[];
};

/**
 * Describes the message ball.v1.FunctionCall.
 * Use `create(FunctionCallSchema)` to create a new message.
 */
export declare const FunctionCallSchema: GenMessage<FunctionCall>;

/**
 * A literal value. Uses oneof for type-safe value representation.
 *
 * @generated from message ball.v1.Literal
 */
export declare type Literal = Message<"ball.v1.Literal"> & {
  /**
   * @generated from oneof ball.v1.Literal.value
   */
  value: {
    /**
     * Integer literal
     *
     * @generated from field: int64 int_value = 1;
     */
    value: bigint;
    case: "intValue";
  } | {
    /**
     * Floating-point literal
     *
     * @generated from field: double double_value = 2;
     */
    value: number;
    case: "doubleValue";
  } | {
    /**
     * String literal
     *
     * @generated from field: string string_value = 3;
     */
    value: string;
    case: "stringValue";
  } | {
    /**
     * Boolean literal
     *
     * @generated from field: bool bool_value = 4;
     */
    value: boolean;
    case: "boolValue";
  } | {
    /**
     * Raw bytes literal
     *
     * @generated from field: bytes bytes_value = 5;
     */
    value: Uint8Array;
    case: "bytesValue";
  } | {
    /**
     * List literal with element expressions
     *
     * @generated from field: ball.v1.ListLiteral list_value = 6;
     */
    value: ListLiteral;
    case: "listValue";
  } | { case: undefined; value?: undefined };
};

/**
 * Describes the message ball.v1.Literal.
 * Use `create(LiteralSchema)` to create a new message.
 */
export declare const LiteralSchema: GenMessage<Literal>;

/**
 * A list of expressions forming a list literal.
 *
 * @generated from message ball.v1.ListLiteral
 */
export declare type ListLiteral = Message<"ball.v1.ListLiteral"> & {
  /**
   * @generated from field: repeated ball.v1.Expression elements = 1;
   */
  elements: Expression[];
};

/**
 * Describes the message ball.v1.ListLiteral.
 * Use `create(ListLiteralSchema)` to create a new message.
 */
export declare const ListLiteralSchema: GenMessage<ListLiteral>;

/**
 * A reference to a named variable.
 * The special name "input" refers to the current function's input parameter.
 *
 * @generated from message ball.v1.Reference
 */
export declare type Reference = Message<"ball.v1.Reference"> & {
  /**
   * @generated from field: string name = 1;
   */
  name: string;

  /**
   * When true, this reference targets the cascade expression's receiver
   * (Dart's `..` chains). Replaces the former "__cascade_self__" sentinel.
   *
   * @generated from field: bool is_cascade_target = 2;
   */
  isCascadeTarget: boolean;
};

/**
 * Describes the message ball.v1.Reference.
 * Use `create(ReferenceSchema)` to create a new message.
 */
export declare const ReferenceSchema: GenMessage<Reference>;

/**
 * Accesses a field of a message-typed expression.
 * Equivalent to `object.field` in most languages.
 *
 * @generated from message ball.v1.FieldAccess
 */
export declare type FieldAccess = Message<"ball.v1.FieldAccess"> & {
  /**
   * The expression that evaluates to a message
   *
   * @generated from field: ball.v1.Expression object = 1;
   */
  object?: Expression | undefined;

  /**
   * The field name to access
   *
   * @generated from field: string field = 2;
   */
  field: string;
};

/**
 * Describes the message ball.v1.FieldAccess.
 * Use `create(FieldAccessSchema)` to create a new message.
 */
export declare const FieldAccessSchema: GenMessage<FieldAccess>;

/**
 * Constructs a new message instance with field values.
 * Equivalent to a constructor call or struct literal.
 *
 * @generated from message ball.v1.MessageCreation
 */
export declare type MessageCreation = Message<"ball.v1.MessageCreation"> & {
  /**
   * The TypeDefinition name to instantiate (empty for anonymous/inline)
   *
   * @generated from field: string type_name = 1;
   */
  typeName: string;

  /**
   * Field values for the new message
   *
   * @generated from field: repeated ball.v1.FieldValuePair fields = 2;
   */
  fields: FieldValuePair[];

  /**
   * Cosmetic hints for target compilers (is_const, dart_prefix, etc.).
   * Follows the same metadata pattern as Module, FunctionDefinition, etc.
   * Stripping metadata must never change what the program computes.
   *
   * @generated from field: google.protobuf.Struct metadata = 3;
   */
  metadata?: JsonObject | undefined;
};

/**
 * Describes the message ball.v1.MessageCreation.
 * Use `create(MessageCreationSchema)` to create a new message.
 */
export declare const MessageCreationSchema: GenMessage<MessageCreation>;

/**
 * A name-value pair for MessageCreation fields.
 *
 * @generated from message ball.v1.FieldValuePair
 */
export declare type FieldValuePair = Message<"ball.v1.FieldValuePair"> & {
  /**
   * Field name
   *
   * @generated from field: string name = 1;
   */
  name: string;

  /**
   * Expression that evaluates to the field's value
   *
   * @generated from field: ball.v1.Expression value = 2;
   */
  value?: Expression | undefined;
};

/**
 * Describes the message ball.v1.FieldValuePair.
 * Use `create(FieldValuePairSchema)` to create a new message.
 */
export declare const FieldValuePairSchema: GenMessage<FieldValuePair>;

/**
 * A block of sequential statements followed by a result expression.
 * The result expression's value is the value of the entire block.
 *
 * @generated from message ball.v1.Block
 */
export declare type Block = Message<"ball.v1.Block"> & {
  /**
   * Statements executed in order
   *
   * @generated from field: repeated ball.v1.Statement statements = 1;
   */
  statements: Statement[];

  /**
   * The final expression whose value is returned
   *
   * @generated from field: ball.v1.Expression result = 2;
   */
  result?: Expression | undefined;
};

/**
 * Describes the message ball.v1.Block.
 * Use `create(BlockSchema)` to create a new message.
 */
export declare const BlockSchema: GenMessage<Block>;

/**
 * A statement within a Block. Either a let-binding or a bare expression.
 *
 * @generated from message ball.v1.Statement
 */
export declare type Statement = Message<"ball.v1.Statement"> & {
  /**
   * @generated from oneof ball.v1.Statement.stmt
   */
  stmt: {
    /**
     * Bind a value to a name for use in subsequent statements/result
     *
     * @generated from field: ball.v1.LetBinding let = 1;
     */
    value: LetBinding;
    case: "let";
  } | {
    /**
     * Evaluate an expression for its side effects
     *
     * @generated from field: ball.v1.Expression expression = 2;
     */
    value: Expression;
    case: "expression";
  } | { case: undefined; value?: undefined };
};

/**
 * Describes the message ball.v1.Statement.
 * Use `create(StatementSchema)` to create a new message.
 */
export declare const StatementSchema: GenMessage<Statement>;

/**
 * Binds a name to the value of an expression.
 * The name is available in all subsequent statements and the block's result.
 *
 * @generated from message ball.v1.LetBinding
 */
export declare type LetBinding = Message<"ball.v1.LetBinding"> & {
  /**
   * Variable name to bind
   *
   * @generated from field: string name = 1;
   */
  name: string;

  /**
   * Expression whose value is bound to the name
   *
   * @generated from field: ball.v1.Expression value = 2;
   */
  value?: Expression | undefined;

  /**
   * Arbitrary metadata for language-specific info (var/final/const, type, etc.)
   *
   * @generated from field: google.protobuf.Struct metadata = 3;
   */
  metadata?: JsonObject | undefined;
};

/**
 * Describes the message ball.v1.LetBinding.
 * Use `create(LetBindingSchema)` to create a new message.
 */
export declare const LetBindingSchema: GenMessage<LetBinding>;

/**
 * A pattern used in match/switch expressions and destructuring declarations.
 * Covers all pattern matching across all target languages (Dart, Rust, Python,
 * Scala, Swift, C#, Java, Kotlin, Haskell, Go, C++).
 * See docs/PATTERN_DESIGN.md for the full cross-language mapping.
 *
 * @generated from message ball.v1.Pattern
 */
export declare type Pattern = Message<"ball.v1.Pattern"> & {
  /**
   * @generated from oneof ball.v1.Pattern.kind
   */
  kind: {
    /**
     * @generated from field: ball.v1.WildcardPattern wildcard = 1;
     */
    value: WildcardPattern;
    case: "wildcard";
  } | {
    /**
     * @generated from field: ball.v1.VariablePattern variable = 2;
     */
    value: VariablePattern;
    case: "variable";
  } | {
    /**
     * @generated from field: ball.v1.ConstantPattern constant = 3;
     */
    value: ConstantPattern;
    case: "constant";
  } | {
    /**
     * @generated from field: ball.v1.TypeTestPattern type_test = 4;
     */
    value: TypeTestPattern;
    case: "typeTest";
  } | {
    /**
     * @generated from field: ball.v1.DestructurePattern destructure = 5;
     */
    value: DestructurePattern;
    case: "destructure";
  } | {
    /**
     * @generated from field: ball.v1.ListPattern list = 6;
     */
    value: ListPattern;
    case: "list";
  } | {
    /**
     * @generated from field: ball.v1.MapPattern map = 7;
     */
    value: MapPattern;
    case: "map";
  } | {
    /**
     * @generated from field: ball.v1.OrPattern or = 8;
     */
    value: OrPattern;
    case: "or";
  } | {
    /**
     * @generated from field: ball.v1.AndPattern and = 9;
     */
    value: AndPattern;
    case: "and";
  } | {
    /**
     * @generated from field: ball.v1.RelationalPattern relational = 10;
     */
    value: RelationalPattern;
    case: "relational";
  } | {
    /**
     * @generated from field: ball.v1.BindingPattern binding = 11;
     */
    value: BindingPattern;
    case: "binding";
  } | {
    /**
     * @generated from field: ball.v1.RestPattern rest = 12;
     */
    value: RestPattern;
    case: "rest";
  } | { case: undefined; value?: undefined };

  /**
   * Guard expression evaluated after structural match succeeds.
   * Universal: Dart 'when', Rust 'if', Python 'if', C# 'when', Scala 'if'.
   *
   * @generated from field: ball.v1.Expression guard = 15;
   */
  guard?: Expression | undefined;

  /**
   * Cosmetic metadata (source location, original syntax form).
   *
   * @generated from field: google.protobuf.Struct metadata = 16;
   */
  metadata?: JsonObject | undefined;
};

/**
 * Describes the message ball.v1.Pattern.
 * Use `create(PatternSchema)` to create a new message.
 */
export declare const PatternSchema: GenMessage<Pattern>;

/**
 * Matches any value, binds nothing. The universal wildcard.
 *
 * @generated from message ball.v1.WildcardPattern
 */
export declare type WildcardPattern = Message<"ball.v1.WildcardPattern"> & {
};

/**
 * Describes the message ball.v1.WildcardPattern.
 * Use `create(WildcardPatternSchema)` to create a new message.
 */
export declare const WildcardPatternSchema: GenMessage<WildcardPattern>;

/**
 * Binds the matched value to a named variable, optionally with a type constraint.
 *
 * @generated from message ball.v1.VariablePattern
 */
export declare type VariablePattern = Message<"ball.v1.VariablePattern"> & {
  /**
   * @generated from field: string name = 1;
   */
  name: string;

  /**
   * @generated from field: ball.v1.TypeRef type = 2;
   */
  type?: TypeRef | undefined;

  /**
   * @generated from field: bool is_final = 3;
   */
  isFinal: boolean;
};

/**
 * Describes the message ball.v1.VariablePattern.
 * Use `create(VariablePatternSchema)` to create a new message.
 */
export declare const VariablePatternSchema: GenMessage<VariablePattern>;

/**
 * Matches a value equal to a constant expression.
 *
 * @generated from message ball.v1.ConstantPattern
 */
export declare type ConstantPattern = Message<"ball.v1.ConstantPattern"> & {
  /**
   * @generated from field: ball.v1.Expression value = 1;
   */
  value?: Expression | undefined;
};

/**
 * Describes the message ball.v1.ConstantPattern.
 * Use `create(ConstantPatternSchema)` to create a new message.
 */
export declare const ConstantPatternSchema: GenMessage<ConstantPattern>;

/**
 * Tests the runtime type of the matched value.
 *
 * @generated from message ball.v1.TypeTestPattern
 */
export declare type TypeTestPattern = Message<"ball.v1.TypeTestPattern"> & {
  /**
   * @generated from field: ball.v1.TypeRef type = 1;
   */
  type?: TypeRef | undefined;

  /**
   * @generated from field: ball.v1.Pattern inner = 2;
   */
  inner?: Pattern | undefined;

  /**
   * @generated from field: bool is_cast = 3;
   */
  isCast: boolean;

  /**
   * @generated from field: bool is_null_check = 4;
   */
  isNullCheck: boolean;

  /**
   * @generated from field: bool is_null_assert = 5;
   */
  isNullAssert: boolean;
};

/**
 * Describes the message ball.v1.TypeTestPattern.
 * Use `create(TypeTestPatternSchema)` to create a new message.
 */
export declare const TypeTestPatternSchema: GenMessage<TypeTestPattern>;

/**
 * Destructures by position and/or name (tuples, records, constructors).
 *
 * @generated from message ball.v1.DestructurePattern
 */
export declare type DestructurePattern = Message<"ball.v1.DestructurePattern"> & {
  /**
   * @generated from field: ball.v1.TypeRef type = 1;
   */
  type?: TypeRef | undefined;

  /**
   * @generated from field: repeated ball.v1.DestructureField fields = 2;
   */
  fields: DestructureField[];
};

/**
 * Describes the message ball.v1.DestructurePattern.
 * Use `create(DestructurePatternSchema)` to create a new message.
 */
export declare const DestructurePatternSchema: GenMessage<DestructurePattern>;

/**
 * @generated from message ball.v1.DestructureField
 */
export declare type DestructureField = Message<"ball.v1.DestructureField"> & {
  /**
   * @generated from field: string name = 1;
   */
  name: string;

  /**
   * @generated from field: ball.v1.Pattern pattern = 2;
   */
  pattern?: Pattern | undefined;
};

/**
 * Describes the message ball.v1.DestructureField.
 * Use `create(DestructureFieldSchema)` to create a new message.
 */
export declare const DestructureFieldSchema: GenMessage<DestructureField>;

/**
 * Matches a sequence (list/array/slice) element by element.
 *
 * @generated from message ball.v1.ListPattern
 */
export declare type ListPattern = Message<"ball.v1.ListPattern"> & {
  /**
   * @generated from field: repeated ball.v1.Pattern elements = 1;
   */
  elements: Pattern[];
};

/**
 * Describes the message ball.v1.ListPattern.
 * Use `create(ListPatternSchema)` to create a new message.
 */
export declare const ListPatternSchema: GenMessage<ListPattern>;

/**
 * Matches a mapping (dictionary/map) by key-value pairs.
 *
 * @generated from message ball.v1.MapPattern
 */
export declare type MapPattern = Message<"ball.v1.MapPattern"> & {
  /**
   * @generated from field: repeated ball.v1.MapPatternEntry entries = 1;
   */
  entries: MapPatternEntry[];

  /**
   * @generated from field: ball.v1.Pattern rest = 2;
   */
  rest?: Pattern | undefined;
};

/**
 * Describes the message ball.v1.MapPattern.
 * Use `create(MapPatternSchema)` to create a new message.
 */
export declare const MapPatternSchema: GenMessage<MapPattern>;

/**
 * @generated from message ball.v1.MapPatternEntry
 */
export declare type MapPatternEntry = Message<"ball.v1.MapPatternEntry"> & {
  /**
   * @generated from field: ball.v1.Expression key = 1;
   */
  key?: Expression | undefined;

  /**
   * @generated from field: ball.v1.Pattern value_pattern = 2;
   */
  valuePattern?: Pattern | undefined;
};

/**
 * Describes the message ball.v1.MapPatternEntry.
 * Use `create(MapPatternEntrySchema)` to create a new message.
 */
export declare const MapPatternEntrySchema: GenMessage<MapPatternEntry>;

/**
 * Matches if any sub-pattern matches (disjunction).
 *
 * @generated from message ball.v1.OrPattern
 */
export declare type OrPattern = Message<"ball.v1.OrPattern"> & {
  /**
   * @generated from field: repeated ball.v1.Pattern patterns = 1;
   */
  patterns: Pattern[];
};

/**
 * Describes the message ball.v1.OrPattern.
 * Use `create(OrPatternSchema)` to create a new message.
 */
export declare const OrPatternSchema: GenMessage<OrPattern>;

/**
 * Matches only if all sub-patterns match (conjunction).
 *
 * @generated from message ball.v1.AndPattern
 */
export declare type AndPattern = Message<"ball.v1.AndPattern"> & {
  /**
   * @generated from field: repeated ball.v1.Pattern patterns = 1;
   */
  patterns: Pattern[];
};

/**
 * Describes the message ball.v1.AndPattern.
 * Use `create(AndPatternSchema)` to create a new message.
 */
export declare const AndPatternSchema: GenMessage<AndPattern>;

/**
 * Compares the matched value against a constant using a relational operator.
 *
 * @generated from message ball.v1.RelationalPattern
 */
export declare type RelationalPattern = Message<"ball.v1.RelationalPattern"> & {
  /**
   * @generated from field: string operator = 1;
   */
  operator: string;

  /**
   * @generated from field: ball.v1.Expression operand = 2;
   */
  operand?: Expression | undefined;
};

/**
 * Describes the message ball.v1.RelationalPattern.
 * Use `create(RelationalPatternSchema)` to create a new message.
 */
export declare const RelationalPatternSchema: GenMessage<RelationalPattern>;

/**
 * Matches the inner pattern and binds the entire value to a name.
 *
 * @generated from message ball.v1.BindingPattern
 */
export declare type BindingPattern = Message<"ball.v1.BindingPattern"> & {
  /**
   * @generated from field: string name = 1;
   */
  name: string;

  /**
   * @generated from field: ball.v1.Pattern pattern = 2;
   */
  pattern?: Pattern | undefined;
};

/**
 * Describes the message ball.v1.BindingPattern.
 * Use `create(BindingPatternSchema)` to create a new message.
 */
export declare const BindingPatternSchema: GenMessage<BindingPattern>;

/**
 * Matches zero or more remaining elements in a sequence or structure.
 *
 * @generated from message ball.v1.RestPattern
 */
export declare type RestPattern = Message<"ball.v1.RestPattern"> & {
  /**
   * @generated from field: ball.v1.Pattern pattern = 1;
   */
  pattern?: Pattern | undefined;
};

/**
 * Describes the message ball.v1.RestPattern.
 * Use `create(RestPatternSchema)` to create a new message.
 */
export declare const RestPatternSchema: GenMessage<RestPattern>;

/**
 * A manifest declares a Ball package's identity, entry point, and dependencies.
 * Analogous to pubspec.yaml, package.json, Cargo.toml.
 * Serialized as `ball.yaml` (human-friendly) or `ball.manifest.json` (proto JSON).
 *
 * @generated from message ball.v1.BallManifest
 */
export declare type BallManifest = Message<"ball.v1.BallManifest"> & {
  /**
   * @generated from field: string name = 1;
   */
  name: string;

  /**
   * @generated from field: string version = 2;
   */
  version: string;

  /**
   * @generated from field: string description = 3;
   */
  description: string;

  /**
   * @generated from field: string entry_module = 4;
   */
  entryModule: string;

  /**
   * @generated from field: string entry_function = 5;
   */
  entryFunction: string;

  /**
   * Direct dependencies required at runtime.
   *
   * @generated from field: repeated ball.v1.ModuleImport dependencies = 6;
   */
  dependencies: ModuleImport[];

  /**
   * Dependencies only needed during development/testing.
   *
   * @generated from field: repeated ball.v1.ModuleImport dev_dependencies = 7;
   */
  devDependencies: ModuleImport[];

  /**
   * Arbitrary metadata (authors, license, homepage, repository, etc.).
   *
   * @generated from field: google.protobuf.Struct metadata = 8;
   */
  metadata?: JsonObject | undefined;
};

/**
 * Describes the message ball.v1.BallManifest.
 * Use `create(BallManifestSchema)` to create a new message.
 */
export declare const BallManifestSchema: GenMessage<BallManifest>;

/**
 * A lockfile pins every transitive dependency to an exact resolved version
 * and content hash. Ensures reproducible builds across machines and time.
 * Serialized as `ball.lock.json` (proto3 JSON, human-readable and diffable).
 *
 * @generated from message ball.v1.BallLockfile
 */
export declare type BallLockfile = Message<"ball.v1.BallLockfile"> & {
  /**
   * @generated from field: repeated ball.v1.ResolvedDependency packages = 1;
   */
  packages: ResolvedDependency[];

  /**
   * Lockfile format version (for forward compatibility).
   *
   * @generated from field: string lock_version = 2;
   */
  lockVersion: string;
};

/**
 * Describes the message ball.v1.BallLockfile.
 * Use `create(BallLockfileSchema)` to create a new message.
 */
export declare const BallLockfileSchema: GenMessage<BallLockfile>;

/**
 * A single resolved dependency in the lockfile.
 *
 * @generated from message ball.v1.ResolvedDependency
 */
export declare type ResolvedDependency = Message<"ball.v1.ResolvedDependency"> & {
  /**
   * Package name (matches ModuleImport.name).
   *
   * @generated from field: string name = 1;
   */
  name: string;

  /**
   * Exact resolved version (not a constraint).
   *
   * @generated from field: string resolved_version = 2;
   */
  resolvedVersion: string;

  /**
   * Content integrity hash: "sha256:<hex>".
   *
   * @generated from field: string integrity = 3;
   */
  integrity: string;

  /**
   * How this dependency was resolved.
   *
   * @generated from oneof ball.v1.ResolvedDependency.resolved_source
   */
  resolvedSource: {
    /**
     * @generated from field: ball.v1.HttpSource http = 4;
     */
    value: HttpSource;
    case: "http";
  } | {
    /**
     * @generated from field: ball.v1.GitSource git = 5;
     */
    value: GitSource;
    case: "git";
  } | {
    /**
     * @generated from field: ball.v1.FileSource file = 6;
     */
    value: FileSource;
    case: "file";
  } | {
    /**
     * @generated from field: ball.v1.RegistrySource registry = 7;
     */
    value: RegistrySource;
    case: "registry";
  } | { case: undefined; value?: undefined };

  /**
   * Names of this package's own dependencies (for the dep graph).
   *
   * @generated from field: repeated string dependency_names = 8;
   */
  dependencyNames: string[];
};

/**
 * Describes the message ball.v1.ResolvedDependency.
 * Use `create(ResolvedDependencySchema)` to create a new message.
 */
export declare const ResolvedDependencySchema: GenMessage<ResolvedDependency>;

/**
 * The output of `ball audit`: a structured report of every side effect
 * a Ball program can perform. Since every side effect in Ball flows
 * through a named base function in a known module, this analysis is
 * provably complete — not heuristic.
 *
 * @generated from message ball.v1.BallCapabilityReport
 */
export declare type BallCapabilityReport = Message<"ball.v1.BallCapabilityReport"> & {
  /**
   * @generated from field: string program_name = 1;
   */
  programName: string;

  /**
   * @generated from field: string program_version = 2;
   */
  programVersion: string;

  /**
   * One entry per capability category found in the program.
   *
   * @generated from field: repeated ball.v1.CapabilityEntry capabilities = 3;
   */
  capabilities: CapabilityEntry[];

  /**
   * Per-function capability breakdown.
   *
   * @generated from field: repeated ball.v1.FunctionCapability functions = 4;
   */
  functions: FunctionCapability[];

  /**
   * Aggregate summary flags.
   *
   * @generated from field: ball.v1.CapabilitySummary summary = 5;
   */
  summary?: CapabilitySummary | undefined;
};

/**
 * Describes the message ball.v1.BallCapabilityReport.
 * Use `create(BallCapabilityReportSchema)` to create a new message.
 */
export declare const BallCapabilityReportSchema: GenMessage<BallCapabilityReport>;

/**
 * A single capability category (e.g. "fs", "io", "memory") with all
 * call sites in the program that trigger it.
 *
 * @generated from message ball.v1.CapabilityEntry
 */
export declare type CapabilityEntry = Message<"ball.v1.CapabilityEntry"> & {
  /**
   * Category name: "pure", "io", "fs", "process", "time", "random",
   *                "memory", "concurrency", "network".
   *
   * @generated from field: string capability = 1;
   */
  capability: string;

  /**
   * Risk level: "none", "low", "medium", "high".
   *
   * @generated from field: string risk_level = 2;
   */
  riskLevel: string;

  /**
   * Every call site in the program that triggers this capability.
   *
   * @generated from field: repeated ball.v1.CallSite call_sites = 3;
   */
  callSites: CallSite[];
};

/**
 * Describes the message ball.v1.CapabilityEntry.
 * Use `create(CapabilityEntrySchema)` to create a new message.
 */
export declare const CapabilityEntrySchema: GenMessage<CapabilityEntry>;

/**
 * A specific location in the program where a capability-bearing base
 * function is called.
 *
 * @generated from message ball.v1.CallSite
 */
export declare type CallSite = Message<"ball.v1.CallSite"> & {
  /**
   * The user module containing the call.
   *
   * @generated from field: string module = 1;
   */
  module: string;

  /**
   * The user function containing the call.
   *
   * @generated from field: string function = 2;
   */
  function: string;

  /**
   * The base module being called (e.g. "std_fs").
   *
   * @generated from field: string callee_module = 3;
   */
  calleeModule: string;

  /**
   * The base function being called (e.g. "file_read").
   *
   * @generated from field: string callee_function = 4;
   */
  calleeFunction: string;
};

/**
 * Describes the message ball.v1.CallSite.
 * Use `create(CallSiteSchema)` to create a new message.
 */
export declare const CallSiteSchema: GenMessage<CallSite>;

/**
 * The capabilities a single function transitively requires.
 *
 * @generated from message ball.v1.FunctionCapability
 */
export declare type FunctionCapability = Message<"ball.v1.FunctionCapability"> & {
  /**
   * @generated from field: string module = 1;
   */
  module: string;

  /**
   * @generated from field: string function = 2;
   */
  function: string;

  /**
   * All capability categories this function (transitively) uses.
   *
   * @generated from field: repeated string capabilities = 3;
   */
  capabilities: string[];
};

/**
 * Describes the message ball.v1.FunctionCapability.
 * Use `create(FunctionCapabilitySchema)` to create a new message.
 */
export declare const FunctionCapabilitySchema: GenMessage<FunctionCapability>;

/**
 * Aggregate boolean summary of a program's capabilities.
 *
 * @generated from message ball.v1.CapabilitySummary
 */
export declare type CapabilitySummary = Message<"ball.v1.CapabilitySummary"> & {
  /**
   * @generated from field: bool is_pure = 1;
   */
  isPure: boolean;

  /**
   * @generated from field: bool reads_filesystem = 2;
   */
  readsFilesystem: boolean;

  /**
   * @generated from field: bool writes_filesystem = 3;
   */
  writesFilesystem: boolean;

  /**
   * @generated from field: bool reads_stdin = 4;
   */
  readsStdin: boolean;

  /**
   * @generated from field: bool writes_stdout = 5;
   */
  writesStdout: boolean;

  /**
   * @generated from field: bool writes_stderr = 6;
   */
  writesStderr: boolean;

  /**
   * @generated from field: bool reads_environment = 7;
   */
  readsEnvironment: boolean;

  /**
   * @generated from field: bool controls_process = 8;
   */
  controlsProcess: boolean;

  /**
   * @generated from field: bool uses_memory = 9;
   */
  usesMemory: boolean;

  /**
   * @generated from field: bool uses_time = 10;
   */
  usesTime: boolean;

  /**
   * @generated from field: bool uses_random = 11;
   */
  usesRandom: boolean;

  /**
   * @generated from field: bool uses_concurrency = 12;
   */
  usesConcurrency: boolean;

  /**
   * @generated from field: bool uses_network = 13;
   */
  usesNetwork: boolean;

  /**
   * @generated from field: int32 total_functions = 14;
   */
  totalFunctions: number;

  /**
   * @generated from field: int32 pure_functions = 15;
   */
  pureFunctions: number;

  /**
   * @generated from field: int32 effectful_functions = 16;
   */
  effectfulFunctions: number;
};

/**
 * Describes the message ball.v1.CapabilitySummary.
 * Use `create(CapabilitySummarySchema)` to create a new message.
 */
export declare const CapabilitySummarySchema: GenMessage<CapabilitySummary>;

/**
 * Supported package registries for RegistrySource.
 *
 * @generated from enum ball.v1.Registry
 */
export enum Registry {
  /**
   * @generated from enum value: REGISTRY_UNSPECIFIED = 0;
   */
  UNSPECIFIED = 0,

  /**
   * pub.dev (Dart/Flutter)
   *
   * @generated from enum value: REGISTRY_PUB = 1;
   */
  PUB = 1,

  /**
   * npmjs.com (JavaScript/TypeScript)
   *
   * @generated from enum value: REGISTRY_NPM = 2;
   */
  NPM = 2,

  /**
   * nuget.org (C#/.NET)
   *
   * @generated from enum value: REGISTRY_NUGET = 3;
   */
  NUGET = 3,

  /**
   * crates.io (Rust)
   *
   * @generated from enum value: REGISTRY_CARGO = 4;
   */
  CARGO = 4,

  /**
   * pypi.org (Python)
   *
   * @generated from enum value: REGISTRY_PYPI = 5;
   */
  PYPI = 5,

  /**
   * Maven Central (Java/Kotlin)
   *
   * @generated from enum value: REGISTRY_MAVEN = 6;
   */
  MAVEN = 6,
}

/**
 * Describes the enum ball.v1.Registry.
 */
export declare const RegistrySchema: GenEnum<Registry>;

/**
 * Serialization format for ball Module data.
 *
 * @generated from enum ball.v1.ModuleEncoding
 */
export enum ModuleEncoding {
  /**
   * Auto-detect from file extension, Content-Type header, or content.
   *   .ball.bin / .ball  → PROTO
   *   .ball.json / .json → JSON
   *   application/x-protobuf → PROTO
   *   application/json → JSON
   *
   * @generated from enum value: MODULE_ENCODING_UNSPECIFIED = 0;
   */
  UNSPECIFIED = 0,

  /**
   * Protobuf binary wire format.
   *
   * @generated from enum value: MODULE_ENCODING_PROTO = 1;
   */
  PROTO = 1,

  /**
   * JSON format (protobuf canonical JSON mapping).
   *
   * @generated from enum value: MODULE_ENCODING_JSON = 2;
   */
  JSON = 2,
}

/**
 * Describes the enum ball.v1.ModuleEncoding.
 */
export declare const ModuleEncodingSchema: GenEnum<ModuleEncoding>;

