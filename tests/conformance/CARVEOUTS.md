...
- `466_std_concurrency_handles` — the FIRST executed fixture for the `std_concurrency`
  ...
  returned nothing — so the whole-corpus parity number said nothing at all about threads,
  mutexes or atomics. Hand-authored because **the Dart encoder routes no Dart syntax to any
  `std_concurrency` function** (zero hits in `dart/encoder/lib/`): `Isolate.spawn` and
  `dart:async` have no encoder routing, so `generate_conformance.dart` can never reach this
  module from a `tests/conformance/src/*.dart`. Generated by
  `dart/encoder/tool/gen_std_gap_fixtures.dart` alongside `389`/`390`/`459`/`462`/`463`/`465`,
  and it is the first fixture there whose golden is AUTHORED rather than derived from the
  reference engine — the tool still runs the engine and fails loudly on a mismatch, so the
  contract could be written down before the engine met it.
  It is the cross-target guard for issue #608's fabricated answers: before the fix
  `atomic_store` discarded the write, `atomic_load` echoed its own input, every
  `atomic_compare_exchange` returned an unconditional `true` (so a CAS retry loop exited on
  its first iteration with the wrong answer), and `thread_spawn` returned the literal `0` —
  identically on all seven engines, since the other six are self-hosted from that source.
  The fixture prints a CAS that must FAIL and the cell value after it; no placeholder can
