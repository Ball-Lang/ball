...
# Ball artifact on each PR. When it finds drift it FAILS (as before) and now also
# uploads the regenerated bytes as the `regenerated-artifacts` workflow artifact
# and prints the apply one-liner. So the normal loop is:
#
...
# nothing. Run the chains below only when you want the artifact locally before
# pushing.
