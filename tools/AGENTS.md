...
| `vcpkg-port/` | — | **Staging area, not a script.** Draft `ports/ball-lang/{vcpkg.json,portfile.cmake}` for the `microsoft/vcpkg` registry submission (issue #368). Inert — not wired into this repo's build or CI. See `vcpkg-port/README.md` for the verified submission flow and the maintainer-only steps left to do. |
