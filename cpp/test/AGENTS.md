...
| `check_declared_base_functions.py` | Declared-drift guard for the compiler's hardcoded `fn == "..."` dispatch (#607), the C++ sibling of `dart/shared/test/std_routed_declarations_test.dart` (#505): every name a module-scoped `compile_*_call` implements must be declared for that module by `tests/conformance/std_coverage.json` (the ALL-module inventory — `dart/shared/std.json` carries only `buildStdModule()`) or be a still-live known gap. `--self-test` runs 6 synthetic cases proving it bites, and a positive floor on the extracted name count is checked FIRST so a regex that stops matching cannot read as a clean bill of health. A successful run REPORTS that extracted count (`extracted N implemented names across 7 dispatchers, floor M`) rather than freezing it in a comment — the same `total_implemented()` the floor uses, so the gate and its report cannot disagree, and the number never rots. Wired into ci.yml's always-on `proto` job — both of its inputs (the C++ dispatch AND the Dart builders) can move it, so a path-filtered `cpp` job would let a declaration-only PR skip it |
| `declared_base_functions_known_gaps.txt` | The frozen, ratchet-only list of names that dispatch is implemented for but no builder declares. MEASURED by running the guard with an empty list, never assumed; it started at 27 and stands at 5. An entry that becomes declared, or that the compiler stops implementing, FAILS — so it can only shrink |

## For AI Agents
- All test files use a **custom `TEST(name)` macro** (defined at the top of each file) — NOT gtest or Catch2. Register tests by defining `TEST(name) { ... }` at file scope; `struct Register_##name` self-registers via constructor. Because registration happens from a static initializer, a binary that lost every `TEST()` would print `0 passed, 0 failed` and still exit 0, so `test_compiler`/`test_ball_dyn`/`test_encoder` carry a **positive floor** (`tests_passed < 1` ⇒ exit 1) alongside the failure check. Keep that floor when adding a new harness. (`test_encoder` was the one harness missing it until the #63 audit; the gap is reproducible by stripping every `TEST()` from the file and running the binary — pre-fix it exits 0.)
- **`cov_*` / `cov63_*` cases are coverage-audit cases and name the miss cluster they close.** Each was written against a specific uncovered line range from Codecov's `cpp` flag `file_report` at a named commit, after establishing that the range is reachable at all. Keep that convention: a coverage case whose comment does not say WHICH lines it targets cannot be re-derived against a fresh report, and a case written without first establishing reachability can silently "cover" dead code. See `.claude/rules/cpp.md`'s "Exclusions" section for when the answer is an `LCOV_EXCL_*` marker instead of a test (almost never — 330 of the 342 audited misses were reachable; the other 12 are the three dominated-dead sites that section names).
...
- **The self-hosted engine has exactly ONE emit shape, `dart/self_host/lib/engine_rt.cpp`** (`cd dart && dart run compiler/tool/compile_engine_cpp.dart`, no flags), `#include`d directly by `test_selfhost_conformance.cpp`. `cpp/test/CMakeLists.txt` used to PREFER a multi-TU `dart/self_host/lib/engine_rt/` shard tree whenever `engine_rt_common.hpp` existed — which was the tool's DEFAULT output and had never compiled; issue #601 deleted that emit, the `BallSelfhostEngine.cmake` OBJECT-library module and the `BALL_SELFHOST_MULTI_TU` include branch. Do not reintroduce a second shape: `regression-gates.yml`'s `C++ self-host: exactly one emit shape (#601)` step fails if an `engine_rt/` tree appears, if the emit tool regains a shape-selecting flag, or if `cpp/` regains an `engine_rt_common.hpp` reference.
...
- `ci.yml`'s `Compiler cache applied (#594)` step
  ...
  Both numbers come from a MACHINE-READABLE form (#660): sccache's
  `Non-cacheable compilations <int>` line, and for ccache the sum of
...
- **The gate step must precede the `full_e2e.sh` smoke steps**, and
  ...
  Read the number from the GATE's step — ubuntu's post-job `ccache -s` block
  shows 4 uncacheable calls that accrue afterwards, from `full_e2e.sh`'s
  compile-and-link smoke, and a reorder would fold them into what the gate
  reads.
