...
## Purpose
Cross-language conformance corpus and supporting test assets. This directory is the operational definition of "done" for any Ball feature — a fixture here validates every full-corpus engine in `.github/workflows/conformance-matrix.yml` simultaneously.
...
| `conformance/*.expected_output.txt` | Golden stdout for each fixture, compared by every full-corpus engine row of `conformance-matrix.yml` |
...
- `conformance/` — The primary CI corpus. Each `NN_<name>.ball.json` + `.expected_output.txt` pair is run against every full-corpus engine row of `conformance-matrix.yml` in CI.
