...
Expression _add(Expression left, int n) =>
    _call('std', 'add', [_field('left', left), _field('right', _intLit(n))]);
...
Expression _writeU8(Expression address, int value) => _call(
  'std_memory',
  'memory_write_u8',
  [_field('address', address), _field('value', _intLit(value))],
);
...
void main() {
  test(
    'compiled memory_realloc moves the block and preserves contents',
    () async {
      final body = Expression()
        ..block = (Block()
          ..statements.addAll([
            ...
            _let(
              'a',
              _call('std_memory', 'memory_alloc', [_field('size', _intLit(4))]),
            ),
