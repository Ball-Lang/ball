...
- **Regenerating the self-hosted CLI core:** run `node --experimental-strip-types tool/regen_compiled_cli.mjs` (compiles `dart/self_host/cli.ball.json` through `compile()`, then adds `export` to every top-level declaration — `cli_core.dart` is a free-function library, so `compile()`'s class-only export logic is not enough — and writes `ts/cli/src/compiled_cli.ts`). The same `Ball Artifact Freshness` job runs it and diffs the result (#580): `compiled_cli.ts` is the only other committed compiled artifact, and it had already drifted at main before the gate existed. Full command in `CLAUDE.md` → Build & Test.
