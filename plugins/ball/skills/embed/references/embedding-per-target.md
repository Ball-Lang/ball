...
`csharp/engine/src/CompiledEngine.cs` is generated only under `-p:SelfHost=true`. In the **default** build, `BallEngine.Run()` unconditionally throws `SelfHostPendingException` (`BallEngine.cs:80-87`) — there is no working `.Run()` at all without that flag. Built WITH the flag, the self-hosted engine runs at Dart parity (`csharp/AGENTS.md`, `Results: 351 passed, 0 failed, 351 total`), but `Run()` still takes **zero parameters**:
...
Runs at Dart parity (`Results: 351 passed, 0 failed, 351 total`; committed compiled artifact, no build tags — see `go/AGENTS.md`):
...
Runs at Dart parity (`python/AGENTS.md`, `Results: 351 passed, 0 failed, 351 total`); the wheel ships the engine's Ball SOURCE and compiles it into a per-user cache on first use:
