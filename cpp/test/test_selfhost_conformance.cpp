...
static bool run_one(const fs::path& program_path, const fs::path& expected_path,
    ...
    try {
        ...
        // Debug: check if std dispatch has functions
        auto dispatch = engine._buildStdDispatch();
        if (dispatch._val.type() == typeid(BallMap)) {
            auto& dm = std::any_cast<const BallMap&>(dispatch._val);
            if (dm.size() == 0) {
                failure_msg = "EMPTY std dispatch!";
                return false;
            }
            // Check if print exists
            auto it = dm.find("print");
            if (it == dm.end()) {
                failure_msg = "No 'print' in std dispatch (has " + std::to_string(dm.size()) + " entries)";
                return false;
            }
        } else {
            failure_msg = "std dispatch is not a map: " + std::string(dispatch._val.type().name());
            return false;
        }
        // Debug: test handles/call directly
        {
            auto& dm = std::any_cast<const BallMap&>(dispatch._val);
            auto printFn = dm.find("print");
            if (printFn != dm.end()) {
                auto fn = BallDyn(printFn->second);
                // Try calling print directly
                auto msg = BallDyn(BallMap{{"message"s, std::any("DIRECT_PRINT_TEST"s)}});
                try {
                    fn(msg);
                } catch (const std::exception& e) {
                    failure_msg = "Direct print call failed: " + std::string(e.what());
                    return false;
                } catch (...) {
                    failure_msg = "Direct print call failed: unknown exception";
                    return false;
                }
                // Also test stdout_ directly
                try {
                    engine.stdout_(BallDyn("STDOUT_TEST"s));
                } catch (const std::exception& e) {
                    failure_msg = "stdout_ call failed: " + std::string(e.what());
                    return false;
                } catch (...) {
                    failure_msg = "stdout_ call failed: unknown exception";
                    return false;
                }
                if (captured->find("STDOUT_TEST") == std::string::npos) {
                    failure_msg = "stdout_ direct call didn't capture (type: " + std::string(engine.stdout_._val.type().name()) + ", captured: '" + *captured + "')";
                    return false;
                }
                // Test _stdPrint directly
                captured->clear();
                auto printMsg = BallDyn(BallMap{{"message"s, std::any(BallDyn("PRINT_TEST"s))}});
                engine._stdPrint(printMsg);
                if (captured->find("PRINT_TEST") == std::string::npos) {
                    failure_msg = "_stdPrint didn't produce output (captured: '" + *captured + "')";
                    return false;
                }
                // Check if captured has output
                if (captured->find("DIRECT_PRINT_TEST") != std::string::npos) {
                    // Direct print works! Issue is in dispatch chain
                } else {
                    failure_msg = "Direct print produced no output (captured: '" + *captured + "')";
                    return false;
                }
            }
        }
        captured->clear(); // Reset for actual test
        // Run the program
        engine.run();
