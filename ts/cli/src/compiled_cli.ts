...
export function ball_cast_assert(ok: boolean, v: any, t: string): boolean {
  if (!ok) {
    throw {
      __type__: 'TypeError',
...
export function analyzeCapabilitiesReachable(program: any): any {
  ...
  let customBaseFns = _collectCustomBaseFns(program.modules, table);
  let fnCaps = {};
  let capSites = {};
  let visited = [];
...
export function _analyzeCapabilitiesCore(ctx: any): any {
  ...
  let customBaseFns = _collectCustomBaseFns(modules, table);
  let functionsOut = [];
  let capSites = {};
  for (const module of modules) {
