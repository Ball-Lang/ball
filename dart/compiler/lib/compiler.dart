...
class DartCompiler {
  ...
  String compile() {
    ...
    return _format(_buildLibrary(mainModule, entryFunc));
  }
  ...
  String compileModule(String moduleName) {
    ...
    return _format(_buildLibrary(module, null));
  }
  ...
  String _emitRaw(cb.Library library) {
    ...
    var raw = '// Generated by ball compiler\n'
  ...
  cb.Library _buildLibrary(
    ...
    for (final func in mainModule.functions) {
      ...
      if (kind == 'method' || kind == 'constructor' || kind == 'static_field') {
        ...
        final afterColon = colonIdx >= 0 ? func.name.substring(colonIdx + 1) : func.name;
        final dotIdx = afterColon.indexOf('.');
        if (dotIdx >= 0) {
          final classKey = func.name.substring(0, (colonIdx >= 0 ? colonIdx + 1 : 0) + dotIdx);
          classMethods.putIfAbsent(classKey, () => []).add(func);
          continue;
        }
        if (dotIdx >= 0) {
    ...
    return cb.Library((b) {
      ...
      if (mainModule.hasMetadata()) {
        ...
        final libAnnots =
            (modMeta['library_annotations'] as List?)?.cast<String>();
        if (libAnnots != null) {
      ...
      for (final td in mainModule.typeDefs) {
        ...
        b.body.add(
            _buildExtensionType(td, classMethods.remove(td.name) ?? []));
      }
    ...
    if (imports is List) {
      for (final imp in imports) {
        ...
        if (configurations != null && configurations.isNotEmpty) {
          ...
          result.add(cb.Directive.import(uri, as: prefix, show: show, hide: hide));
    ...
    if (exports is List) {
      for (final exp in exports) {
        ...
        if (configurations != null && configurations.isNotEmpty) {
          ...
          result.add(cb.Directive.export(
            uri,
            show: show,
            hide: hide,
          ));
  ...
  cb.Class _buildClass(TypeDefinition td, List<FunctionDefinition> methods) {
    ...
    final isPlain = (kind == null || kind == 'class') &&
        !isAbstract &&
        !isSealed &&
        !isFinal &&
        !isBase &&
    ...
    return cb.Class((b) {
      ...
      for (final method in methods) {
        ...
        if (mKind == 'constructor') {
          b.constructors
              .add(_buildConstructor(descriptor.name, method, mMeta));
        } else {
    ...
    return cb.Class((b) {
      ...
      if (type.field.isNotEmpty) {
        b.constructors.add(cb.Constructor((c) {
          for (final field in type.field) {
            c.optionalParameters.add(cb.Parameter((p) {
      ...
      if (isAbstract) {
        ...
        if (modifier != 'final' && modifier != 'const') {
          addMethod(cb.Method((mb) {
            ...
            mb.requiredParameters.add(cb.Parameter((p) {
              p.name = 'value';
              if (hasExplicitType && fieldType != null) {
  ...
  cb.Field _buildStaticField(FunctionDefinition func) {
    ...
    return cb.Field((fb) {
      fb.name = fieldName;
      fb.static = true;
      if (func.outputType.isNotEmpty) fb.type = cb.refer(_dartType(func.outputType));
      ...
      if (func.hasBody()) fb.assignment = cb.Code(_compileExpression(func.body));
    });
  }
  ...
  cb.Extension _buildExtension(
      TypeDefinition td, List<FunctionDefinition> methods) {
    final meta = _metaFromTd(td);
    final on = meta['on'] as String?;
    final typeParamsStr = _typeParamsStr(meta);
    final doc = meta['doc'] as String?;
    final displayName =
        td.name == '_unnamed_extension' ? '' : _dartType(td.name);
  ...
  cb.ExtensionType _buildExtensionType(
    ...
    return cb.ExtensionType((b) {
      ...
      b.representationDeclaration =
          cb.RepresentationDeclaration((rb) {
        rb.name = repField;
  ...
  cb.Method _buildMethodFromMeta(
    ...
    return cb.Method((b) {
      ...
      b.name = isOperator ? 'operator $name$typeParamsStr' : '$name$typeParamsStr';
      if (doc != null) b.docs.add(doc);
      if (annotations != null) {
      ...
      if (func.outputType.isNotEmpty) b.returns = cb.refer(_dartType(func.outputType));
      ...
      if (!isAbstract && !isExternal && func.hasBody()) {
        if (isExpressionBody) {
          b.lambda = true;
          b.body = cb.Code(_compileExpression(func.body));
        } else {
          b.body = cb.Code(_captureBody(
  ...
  cb.Constructor _buildConstructor(
    ...
    return cb.Constructor((b) {
      ...
      if (redirectsTo != null) {
        ...
        b.initializers
            .addAll(_buildInitializerList(initializers).map(cb.Code.new));
      }
  ...
  String _buildTopLevelVarStr(FunctionDefinition func) {
    ...
    buf.write(func.name);
    if (func.hasBody()) buf.write(' = ${_compileExpression(func.body)}');
    buf.write(';');
    return buf.toString();
  }
  ...
  Map<String, Object?> _metaFromTd(TypeDefinition td) {
    final meta =
        td.hasMetadata() ? _structToMap(td.metadata) : <String, Object?>{};
  ...
  void _generateFunctionBody(Expression body, bool hasReturn) {
    ...
    if (_isStdControl(body) &&
        !(hasReturn && body.call.function == 'assign')) {
      _generateStdStatement(body.call, hasReturn);
      return;
    }
    _wl(hasReturn
        ? 'return ${_compileExpression(body)};'
        : '${_compileExpression(body)};');
  }
  ...
  void _generateBlockStatements(Block block, bool hasReturn) {
    ...
    if (block.hasResult()) {
      if (_isStdControl(block.result) &&
        ...
        _wl(hasReturn
            ? 'return ${_compileExpression(block.result)};'
            : '${_compileExpression(block.result)};');
      }
    }
  }
  ...
  void _generateLocalFunction(String name, FunctionDefinition func) {
    ...
    final returnType =
        func.outputType.isEmpty ? null : _dartType(func.outputType);
    final isAsync = meta['is_async'] == true;
    final isAsyncStar = meta['is_async_star'] == true;
    final isSyncStar = meta['is_sync_star'] == true;
    ...
    if (isExprBody && func.hasBody()) {
      _wl('$sig => ${_compileExpression(func.body)};');
    } else {
      _wl('$sig {');
      _depth++;
      if (func.hasBody()) {
  ...
  static bool _isBaseModule(String module) =>
      module == 'std' || module == 'dart_std' || module == 'std_memory';
  ...
  bool _isStdControl(Expression expr) {
    ...
    return const {
      ...
      'try', 'return', 'break', 'continue', 'assert', 'yield',
      'yield_each', 'assign', 'labeled', 'throw', 'rethrow',
  ...
  void _generateStdStatement(FunctionCall call, bool hasReturn) {
    ...
    switch (call.function) {
      ...
      case 'yield':
        final v = fields['value'];
        _wl('yield ${v != null ? _compileExpression(v) : "null"};');
      case 'yield_each':
        final v = fields['value'];
        _wl('yield* ${v != null ? _compileExpression(v) : "null"};');
      case 'assign':
      ...
      case 'throw':
        final v = fields['value'];
        _wl('throw ${v != null ? _compileExpression(v) : "null"};');
      case 'rethrow':
        _wl('rethrow;');
      default:
  ...
  String _compileStdStatementToString(FunctionCall call, bool hasReturn) {
    ...
    switch (call.function) {
      case 'return':
        final v = fields['value'];
        return v != null ? 'return ${_compileExpression(v)};\n' : 'return;\n';
      case 'break':
        final label = _stringFieldValue(fields, 'label');
        return (label != null && label.isNotEmpty ? 'break $label;\n' : 'break;\n');
      case 'continue':
        final label = _stringFieldValue(fields, 'label');
      ...
      case 'throw':
        final v = fields['value'];
        return 'throw ${v != null ? _compileExpression(v) : "null"};\n';
      case 'rethrow':
        return 'rethrow;\n';
      case 'assign':
  ...
  void _generateIf(Map<String, Expression> fields, bool hasReturn) {
    ...
    if (casePattern != null && casePattern.isNotEmpty) {
      _wl('if (${_compileExpression(cond)} case $casePattern) {');
    } else {
      _wl('if (${_compileExpression(cond)}) {');
    }
    _depth++;
  ...
  void _generateBranchBody(Expression expr, bool hasReturn) {
    if (expr.whichExpr() == Expression_Expr.block) {
      ...
      _wl(hasReturn
          ? 'return ${_compileExpression(expr)};'
  ...
  void _generateFor(Map<String, Expression> fields) {
    ...
    final init = initExpr != null
        ? (_stringFieldValue(fields, 'init') ?? _compileExpression(initExpr))
        : '';
    final condStr =
        fields['condition'] != null ? _compileExpression(fields['condition']!) : '';
    final updateStr =
        fields['update'] != null ? _compileExpression(fields['update']!) : '';
  ...
  void _generateForIn(Map<String, Expression> fields) {
    ...
    _wl('${awaitKw}for ($typeDecl in '
        '${iterable != null ? _compileExpression(iterable) : "[]"}) {');
    _depth++;
    if (body != null) _generateBranchBody(body, false);
    _depth--;
    _wl('}');
  ...
  void _generateWhile(Map<String, Expression> fields) {
    final cond = fields['condition'];
    final body = fields['body'];
    _wl('while (${cond != null ? _compileExpression(cond) : "true"}) {');
    _depth++;
    if (body != null) _generateBranchBody(body, false);
    _depth--;
    _wl('}');
  ...
  void _generateSwitch(Map<String, Expression> fields) {
    ...
    _wl('switch (${subject != null ? _compileExpression(subject) : "null"}) {');
    _depth++;
    if (cases != null &&
  ...
  void _generateSwitchCase(Expression caseExpr) {
    ...
    if (isDefault) {
      ...
      _wl('case ${_compileExpression(value)}:');
    } else {
      return;
    }
    _depth++;
  ...
  void _generateAssert(Map<String, Expression> fields) {
    ...
    if (cond == null) return;
    _wl(msg != null
        ? 'assert(${_compileExpression(cond)}, ${_compileExpression(msg)});'
        : 'assert(${_compileExpression(cond)});');
  }
  ...
  void _generateAssign(Map<String, Expression> fields) {
    ...
    _wl('${_compileExpression(target)} $assignOp ${_compileExpression(value)};');
  }
  ...
  String _compileExpression(Expression expr) => switch (expr.whichExpr()) {
        Expression_Expr.call => _compileCall(expr.call),
        Expression_Expr.literal => _compileLiteral(expr.literal),
        Expression_Expr.reference => expr.reference.name,
        Expression_Expr.fieldAccess =>
          _compileFieldAccess(expr.fieldAccess),
        Expression_Expr.messageCreation =>
          _compileMessageCreation(expr.messageCreation),
        Expression_Expr.block => _compileBlockExpression(expr.block),
        Expression_Expr.lambda => _compileLambda(expr.lambda),
        Expression_Expr.notSet => '/* unknown expression */',
      };

  String _compileFieldAccess(FieldAccess fa) {
    final objStr = _compileExpression(fa.object);
    // __cascade_self__ is a sentinel that means "no explicit receiver" inside
    // a cascade section — emit just the field name.
    if (objStr == '__cascade_self__') return fa.field_2;
    return '$objStr.${fa.field_2}';
  }
  ...
  String _compileCall(FunctionCall call) {
    ...
    if (call.hasInput() &&
      ...
      if (selfField != null) {
        ...
        final typeArgsField = fields.where((f) => f.name == '__type_args__').firstOrNull;
        final typeArgs = typeArgsField != null
            ? typeArgsField.value.literal.stringValue
            : '';
        final remaining = fields.where((f) => f.name != 'self' && f.name != '__type_args__').toList();
        final selfStr = _compileExpression(selfField.value);
    ...
    if (call.hasInput()) {
      ...
      if (inp.whichExpr() == Expression_Expr.messageCreation &&
        ...
        final typeArgsField = allFields.where((f) => f.name == '__type_args__').firstOrNull;
        final typeArgs = typeArgsField != null
            ? typeArgsField.value.literal.stringValue
            : '';
      ...
      return '$modulePrefix${call.function}(${_compileExpression(inp)})';
    }
  ...
  String _compileBaseCall(FunctionCall call) {
    ...
    return switch (call.function) {
      ...
      'paren' => '(${_compileExpression(f['value']!)})',
  ...
  String _compileMemoryCall(FunctionCall call) {
    ...
    return switch (call.function) {
      ...
      // ── Typed reads ──
      'memory_read_i8' =>
        '_ballMemory.getInt8(${_addrExpr(f)})',
      'memory_read_u8' =>
        '_ballMemory.getUint8(${_addrExpr(f)})',
      'memory_read_i16' =>
      ...
      'stack_pop_frame' =>
        '_ballStackPtr = _ballStackFrames.removeLast()',
  ...
  String _addrExpr(Map<String, Expression> f) {
    ...
    return a != null ? _compileExpression(a) : '0';
  }

  String _valExpr(Map<String, Expression> f) {
    final v = f['value'];
    return v != null ? _compileExpression(v) : '0';
  }

  String _memAlloc(Map<String, Expression> f) {
    final size = f['size'];
    final sizeStr = size != null ? _compileExpression(size) : '0';
  ...
  String _memCopy(Map<String, Expression> f) {
    ...
    final d = dest != null ? _compileExpression(dest) : '0';
    final s = src != null ? _compileExpression(src) : '0';
    final n = size != null ? _compileExpression(size) : '0';
  ...
  String _memSet(Map<String, Expression> f) {
    ...
    final a = addr != null ? _compileExpression(addr) : '0';
    final v = val != null ? _compileExpression(val) : '0';
    final n = size != null ? _compileExpression(size) : '0';
    return '(() { for (var __i = 0; __i < $n; __i++) '
  ...
  String _memCompare(Map<String, Expression> f) {
    ...
    final aStr = a != null ? _compileExpression(a) : '0';
    final bStr = b != null ? _compileExpression(b) : '0';
    final n = size != null ? _compileExpression(size) : '0';
    return '(() { for (var __i = 0; __i < $n; __i++) { '
  ...
  String _ptrArith(Map<String, Expression> f, String op) {
    final addr = f['address'], offset = f['offset'], elemSize = f['element_size'];
    final a = addr != null ? _compileExpression(addr) : '0';
    final o = offset != null ? _compileExpression(offset) : '0';
    final es = elemSize != null ? _compileExpression(elemSize) : '1';
    return '($a $op ($o * $es))';
  }
  ...
  String _memDeref(Map<String, Expression> f) {
    final ptr = f['pointer'];
    final ptrStr = ptr != null ? _compileExpression(ptr) : '0';
  ...
  String _compileBasePrint(Map<String, Expression> f) {
    final msg = f['message'];
    return msg != null ? 'print(${_compileExpression(msg)})' : 'print()';
  }

  String _binOp(Map<String, Expression> f, String op) {
    final l = f['left'], r = f['right'];
    return l != null && r != null
        ? '(${_compileExpression(l)} $op ${_compileExpression(r)})'
        : '/* invalid $op */';
  }

  String _prefixOp(Map<String, Expression> f, String op) {
    final v = f['value'];
    return v != null ? '($op${_compileExpression(v)})' : '/* invalid $op */';
  }
  ...
  String _postfixMut(Map<String, Expression> f, String op) {
    final v = f['value'];
    return v != null ? '(${_compileExpression(v)}$op)' : '/* invalid $op */';
  }

  String _awaitExpr(Map<String, Expression> f) {
    final v = f['value'];
    return v != null ? 'await (${_compileExpression(v)})' : 'await null';
  }

  String _throwExpr(Map<String, Expression> f) {
    final v = f['value'];
    return v != null ? '(throw ${_compileExpression(v)})' : '(throw null)';
  }

  String _compileSpread(Map<String, Expression> f, String op) {
    final v = f['value'];
    return v != null ? '$op${_compileExpression(v)}' : '${op}null';
  }
  ...
  String _compileToString(Map<String, Expression> f) {
    ...
    return '${_compileExpression(v)}.toString()';
  }

  String _methodCallExpr(Map<String, Expression> f, String method) {
    final v = f['value'];
    return v != null
        ? '${_compileExpression(v)}.$method'
        : '/* invalid .$method */';
  }
  ...
  String _methodCall2(Map<String, Expression> f, String method) {
    ...
    return l != null && r != null
        ? '${_compileExpression(l)}.$method(${_compileExpression(r)})'
        : '/* invalid $method() */';
  }
  ...
  String _typeOp(Map<String, Expression> f, String op) {
    ...
    return v != null && t != null
        ? '(${_compileExpression(v)} $op $t)'
        : '/* invalid $op */';
  }

  String _mathFunc(Map<String, Expression> f, String func) {
    final v = f['value'];
    return v != null ? '$func(${_compileExpression(v)})' : '/* invalid $func */';
  }

  String _mathBinary(Map<String, Expression> f, String func) {
    final l = f['left'], r = f['right'];
    return l != null && r != null
        ? '$func(${_compileExpression(l)}, ${_compileExpression(r)})'
        : '/* invalid $func */';
  }
  ...
  String _compileAssign(Map<String, Expression> f) {
    ...
    return '$targetStr $op ${_compileExpression(value)}';
  }
  ...
  String _compileNullAwareCall(FunctionCall call) {
    ...
    final args = call.input.messageCreation.fields
        .where((f) => f.name != 'target' && f.name != 'method' && f.name != '__type_args__')
        .toList();
    return '${_compileExpression(t)}?.$method$typeArgs(${_compileArgs(args)})';
  }
  ...
  String _compileInlineIf(Map<String, Expression> f) {
    ...
    return '(${_compileExpression(c)} ? ${_compileExpression(t)} '
        ': ${e != null ? _compileExpression(e) : "null"})';
  }

  String _compileIndex(Map<String, Expression> f) {
    final t = f['target'], i = f['index'];
    return t != null && i != null
        ? '${_compileExpression(t)}[${_compileExpression(i)}]'
        : '/* invalid [] */';
  }
  ...
  String _compileCascade(Map<String, Expression> f) {
    ...
    final buf = StringBuffer(_compileExpression(t));
    ...
    if (sections != null &&
      ...
      for (final s in sections.literal.listValue.elements) {
        ...
        final raw = _compileExpression(s);
  ...
  String _compileInvoke(FunctionCall call) {
    ...
    final args = call.input.messageCreation.fields
        ...
        .toList();
    return '${_compileExpression(callee)}(${_compileArgs(args)})';
  }

  String _compileMapCreate(FunctionCall call) {
    final allFields = call.hasInput() &&
            call.input.whichExpr() == Expression_Expr.messageCreation
    ...
    for (final field in entries) {
      ...
      if (field.name == 'entry' &&
        ...
        if (k != null && v != null) {
          buf.write('${_compileExpression(k)}: ${_compileExpression(v)}');
        }
      } else {
        buf.write(_compileExpression(field.value));
      }
    }
    buf.write('}');
    return buf.toString();
  ...
  String _compileSetCreate(Map<String, Expression> f) {
    ...
    if (elements == null) {
      ...
      return typePrefix.isEmpty ? '{}' : typePrefix == 'Set<dynamic>' || typePrefix == 'Map<dynamic, dynamic>' ? '{}' : '$typePrefix{}';
    }
    if (elements.whichExpr() == Expression_Expr.literal &&
        elements.literal.whichValue() == Literal_Value.listValue) {
      final items = elements.literal.listValue.elements.map(_compileExpression).join(', ');
      if (items.isEmpty) {
        return typePrefix.isEmpty ? '{}' : typePrefix == 'Set<dynamic>' || typePrefix == 'Map<dynamic, dynamic>' ? '{}' : '$typePrefix{}';
      }
  ...
  String _compileTypedList(Map<String, Expression> f) {
    ...
    if (elements != null &&
        ...
        elements.literal.whichValue() == Literal_Value.listValue) {
      return '$typeArgs[${elements.literal.listValue.elements.map(_compileExpression).join(', ')}]';
    }
    return '$typeArgs[]';
  }
    ...
    if (fieldsValue.whichExpr() == Expression_Expr.literal &&
      ...
      for (final entry in fieldsValue.literal.listValue.elements) {
        ...
        if (nameExpr.whichExpr() != Expression_Expr.notSet && val != null) {
          ...
          parts.add(nameStr.startsWith('arg')
              ? _compileExpression(val)
              : '$nameStr: ${_compileExpression(val)}');
        }
      }
      return '(${parts.join(', ')})';
    }
  ...
  String _compileCollectionIf(Map<String, Expression> f) {
    ...
    final buf = StringBuffer(
        'if (${_compileExpression(c)}) ${_compileExpression(t)}');
    if (e != null) buf.write(' else ${_compileExpression(e)}');
    return buf.toString();
  }
  ...
  String _compileCollectionFor(Map<String, Expression> f) {
    ...
    return 'for (final $variable in ${_compileExpression(iterable)}) '
        '${_compileExpression(body)}';
  }
  ...
  String _compileSwitchExpr(Map<String, Expression> f) {
    ...
    final buf = StringBuffer('switch (${_compileExpression(subject)}) {\n');
    final cases = f['cases'];
    if (cases != null &&
        cases.whichExpr() == Expression_Expr.literal &&
        cases.literal.whichValue() == Literal_Value.listValue) {
      for (final c in cases.literal.listValue.elements) {
        ...
        if (pattern != null && body != null) {
          buf.write('$pattern => ${_compileExpression(body)},\n');
        }
      }
    }
    buf.write('}');
  ...
  String _compileSubstring(Map<String, Expression> f) {
    ...
    final endStr = e != null ? ', ${_compileExpression(e)}' : '';
    return '${_compileExpression(v)}.substring(${_compileExpression(s)}$endStr)';
  }

  String _compileStringCharCodeAt(Map<String, Expression> f) {
    final t = f['target'], i = f['index'];
    return t != null && i != null
        ? '${_compileExpression(t)}.codeUnitAt(${_compileExpression(i)})'
        : '/* invalid codeUnitAt */';
  }
  ...
  String _compileStringReplace(Map<String, Expression> f, String method) {
    ...
    return v != null && from != null && to != null
        ? '${_compileExpression(v)}.$method(${_compileExpression(from)}, ${_compileExpression(to)})'
        : '/* invalid $method */';
  }
  ...
  String _compileStringPad(Map<String, Expression> f, String method) {
    ...
    final padStr = p != null ? ', ${_compileExpression(p)}' : '';
    return '${_compileExpression(v)}.$method(${_compileExpression(w)}$padStr)';
  }
  ...
  String _compileRegexMatch(Map<String, Expression> f) {
    ...
    return input != null && pattern != null
        ? 'RegExp(${_compileExpression(pattern)}).hasMatch(${_compileExpression(input)})'
        : '/* invalid regex_match */';
  }
  ...
  String _compileRegexFind(Map<String, Expression> f, {required bool all}) {
    ...
    if (all) {
      return 'RegExp(${_compileExpression(pattern)})'
          '.allMatches(${_compileExpression(input)})'
          '.map((m) => m.group(0)!).toList()';
    }
    return 'RegExp(${_compileExpression(pattern)})'
        '.firstMatch(${_compileExpression(input)})?.group(0)';
  }

  String _compileRegexReplace(Map<String, Expression> f, String method) {
    final v = f['value'], from = f['from'], to = f['to'];
    return v != null && from != null && to != null
        ? '${_compileExpression(v)}.$method(RegExp(${_compileExpression(from)}), ${_compileExpression(to)})'
  ...
  String _compileMathClamp(Map<String, Expression> f) {
    ...
    return v != null && mn != null && mx != null
        ? '${_compileExpression(v)}'
            '.clamp(${_compileExpression(mn)}, ${_compileExpression(mx)})'
        : '/* invalid clamp */';
  }
  ...
  String _compileMathLcm(Map<String, Expression> f) {
    ...
    final ls = _compileExpression(l), rs = _compileExpression(r);
    return '(($ls * $rs).abs() ~/ $ls.gcd($rs))';
  }

  // ── Literals ─────────────────────────────────────────────────

  String _compileLiteral(Literal lit) => switch (lit.whichValue()) {
        Literal_Value.intValue => '${lit.intValue.toInt()}',
        Literal_Value.doubleValue => _formatDouble(lit.doubleValue),
        Literal_Value.stringValue =>
          "'${_escapeDartString(lit.stringValue)}'",
        Literal_Value.boolValue => '${lit.boolValue}',
        Literal_Value.bytesValue => '${lit.bytesValue}',
        Literal_Value.listValue =>
          '[${lit.listValue.elements.map(_compileExpression).join(', ')}]',
  ...
  String _compileMessageCreation(MessageCreation msg) {
    if (msg.typeName.isEmpty) {
      ...
      if (msg.fields.length == 2 &&
        ...
        final k = _compileExpression(msg.fields[0].value);
        final v = _compileExpression(msg.fields[1].value);
        return '$k: $v';
      }
    ...
    final moduleOrClass = colonIdx >= 0 ? rawTypeName.substring(0, colonIdx) : '';
    final afterColon = colonIdx >= 0 ? rawTypeName.substring(colonIdx + 1) : rawTypeName;
    ...
    final constField = msg.fields
        ...
        .firstOrNull;
    final isConst = constField != null &&
        constField.value.whichExpr() == Expression_Expr.literal &&
        constField.value.literal.boolValue == true;
    final actualFields =
        msg.fields.where((f) => f.name != '__type_args__' && f.name != '__const__').toList();
  ...
  String _compileBlockExpression(Block block) {
    ...
    for (final stmt in block.statements) {
      if (stmt.whichStmt() == Statement_Stmt.let) {
        buf.write('${_letDeclKeyword(stmt.let)} ${stmt.let.name} = '
            '${_compileExpression(stmt.let.value)};\n');
        ...
        if (_isStdControl(expr)) {
          ...
          buf.write('${_compileExpression(expr)};\n');
        }
      }
    }
    if (block.hasResult()) {
      buf.write('return ${_compileExpression(block.result)};\n');
    }
    buf.write('})()');
    return buf.toString();
  }
  ...
  String _compileLambda(FunctionDefinition func) {
    ...
    if (isExprBody) {
      buf.write(' => ${_compileExpression(func.body)}');
    } else {
  ...
  String _compileArgs(List<FieldValuePair> fields) {
    ...
    for (final field in fields) {
      parts.add(field.name.startsWith('arg')
          ? _compileExpression(field.value)
          : '${field.name}: ${_compileExpression(field.value)}');
    }
    return parts.join(', ');
  }
  ...
  String _protoTypeToDartType(google.FieldDescriptorProto_Type type) =>
      switch (type) {
        ...
        google.FieldDescriptorProto_Type.TYPE_FLOAT =>
          'double',
        ...
        google.FieldDescriptorProto_Type.TYPE_SINT64 =>
          'int',
  ...
  String _dartFieldName(String name) {
    const reserved = {
      ...
      'part', 'return', 'this', 'super', 'new',
      'null', 'true', 'false', 'var', 'final', 'const',
      'void', 'if', 'else', 'for', 'while', 'do', 'switch',
      'case', 'default', 'break', 'continue', 'try', 'catch',
      'finally', 'throw', 'assert', 'in', 'is', 'as',
    };
  ...
  Map<String, Object?> _structToMap(structpb.Struct struct) => {
        for (final e in struct.fields.entries)
          e.key: _structValueToObject(e.value),
      };
  ...
  static String _memberName(String funcName) {
    ...
    final afterColon = colonIdx >= 0 ? funcName.substring(colonIdx + 1) : funcName;
    final dotIdx = afterColon.indexOf('.');
