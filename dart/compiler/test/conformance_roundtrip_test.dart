...
void main() async {
  ...
  // into a single all-green check at the end — every leg must pass, with no
  // tolerated-failure baseline.
  test(
    'all legs round-trip (${cases.length} programs × ${_legs.length} legs)',
    () async {
      ...
      try {
        ...
        if (failures.isNotEmpty) {
          msg.writeln(
            '${failures.length} leg(s) failed to round-trip — every leg must '
            'pass (no tolerated failures):',
          );
          for (final f in failures) {
            msg.writeln('  + $f');
          }
        }
        expect(failures, isEmpty, reason: msg.toString());
      } finally {
        try {
