...
| `ballrt/flow.py` | `break`/`continue`/`return`/`throw` as exceptions + the `ret`/`brk`/`cont`/`throw`/`rethrow` helper calls that raise them, and `catch_matches`/`exception_type_name` — the typed-`on T catch` clause-selection rule the compiler's dispatch chain calls (issue #724; the sibling of Go's `ballrt.CatchMatches` and Rust's `ball_catch_matches`). |
