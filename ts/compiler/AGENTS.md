...
Ball → TypeScript compiler. Consumes a `Program` (proto3-JSON object) and emits idiomatic TypeScript source via `ts-morph`. Also used internally to regenerate `ts/engine/src/compiled_engine.ts` from the self-hosted engine Ball source.
...
| `src/preamble.ts` | `TS_RUNTIME_PREAMBLE` — Dart-flavored polyfills installed on `Object.prototype` (e.g. `whichExpr()`, `hasBody()`) so compiled Dart-origin code can call proto-style methods on plain JSON objects. |
