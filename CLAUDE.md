...
- **Self-hosted engine** (`dart/self_host/lib/engine_rt.cpp`): The Dart engine compiled to C++ via the Ball C++ compiler. Compiles and runs — **108/175 conformance tests pass (2026-05-24)**, up from 72 after a compiler-correctness wave: positional ctor args now resolve to param names (fixed the FlowSignal `arg0`/`kind` key mismatch that lost every early `return`); try/finally runs cleanup on the return path and no longer swallows exceptions (was leaking `_expressionDepth` → null past ~100 nested calls); thrown exception payloads are preserved through throw/catch; `List.filled` unwraps its BallDyn length. See `docs/SELF_HOST_STATUS.md` for the full history. Largest remaining lever: reference-semantic (shared_ptr-backed) `BallList`/`BallMap`/`BallObject` — value-type containers copy on read, so in-place mutation (sorts, OOP setters) is lost. Run the real count with `bash cpp/test/run_selfhost_tally.sh` (the in-process harness dies on the first stack-overflowing fixture). Build: `cmake --build build --target test_selfhost_conformance`.
